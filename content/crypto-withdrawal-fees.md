---
title: 暗号資産出金手数料の比較
description: 国内暗号資産取引所の暗号資産出金手数料を、全銘柄・ネットワーク別に検索して比較できます。
date: 2026-05-09
updated: 2026-05-09
author: 国内暗号資産取引所ナビ
slug: crypto-withdrawal-fees
path: /learn/crypto-withdrawal-fees
---

暗号資産を取引所から外部ウォレットや他サービスへ移すときは、出金手数料、対応ネットワーク、最小出金数量をセットで確認します。同じ BTC や ETH でも、取引所やネットワークによって手数料が大きく変わります。

このページでは、OKJ、Coincheck、bitFlyer、bitbank、GMOコイン、Binance Japan、BitTrade の暗号資産出金手数料を、公式公開ページまたは公式公開APIで確認できる範囲で全銘柄・ネットワーク別に整理します。確認日は 2026年5月9日です。

## 全銘柄の比較表

<div class="crypto-withdrawal-tool" data-crypto-withdrawal-tool>
  <div class="cw-empty-panel">比較データを読み込んでいます</div>
</div>
<script src="/js/crypto-withdrawal-fees.js?v=4"></script>

## 見る順番

最初に銘柄で絞り込み、同じ銘柄の中で出金先が対応しているネットワークだけを見ます。Binance Japan、OKJ、bitbank のように複数ネットワークがある場合、最安のネットワークが必ずしも自分の送付先で使えるとは限りません。

次に、出金手数料と最小出金数量を合わせて見ます。少額を送る場合は、手数料が安くても最小出金数量を満たせないことがあります。表の最小出金欄が空欄の取引所は、出金画面または公式ヘルプで最終確認してください。

最後に、出金先の入金条件を確認します。XRP などタグやメモが必要な銘柄、Ethereum 系のレイヤー2、BEP20、Polygon、Solana などは、出金元と受取側のネットワークが一致している必要があります。

## 比較対象と公式確認先

- [OKJ 入出金・入出庫](https://www.okcoin.jp/pages/products/deposit-withdraw.html)
- [Coincheck 手数料](https://coincheck.com/ja/info/fee)
- [bitFlyer 手数料一覧](https://bitflyer.com/ja-jp/s/commission)
- [bitbank 手数料](https://bitbank.cc/guide/fee)
- [GMOコイン 入出金](https://coin.z.com/jp/corp/guide/deposit-withdrawal/)
- [Binance 暗号資産手数料](https://www.binance.com/ja/fee/cryptoFee)
- [BitTrade 手数料](https://www.bittrade.co.jp/ja-jp/support/fee/)

## 日本円出金と合わせて見る

取引所選びでは、暗号資産の出金手数料だけでなく、日本円を銀行口座へ戻すコストも確認します。日本円の条件は [日本円出金手数料の比較](/learn/jpy-withdrawal-fees) にまとめています。

売買コスト全体を整理したい場合は [暗号資産の手数料の見方](/learn/crypto-fees) から確認し、実際に買う銘柄は [銘柄深掘り一覧](/markets) や [取引コスト計算（板シミュレーター）](/simulator?market=BTC-JPY&side=buy&amountType=jpy&amount=100000) で比較してください。
