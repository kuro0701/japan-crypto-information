...
const PAGE_CONFIGS = {
  home: {
    ...
    path: '/',
    name: SITE_NAME,
    title: '国内暗号資産取引所 比較ホーム｜板・スプレッド・手数料・出来高｜国内暗号資産取引所ナビ',
    description: '国内暗号資産取引所の板・スプレッド・手数料を比較し、どこで買うと実質コストが低いかを確認できる比較ホームです。',
    ogImagePath: '/ogp/default.png',
  },
  simulator: {
    pageId: 'simulator',
    path: '/simulator',
    name: '取引コスト計算（板シミュレーター）',
    title: '取引コスト計算（板シミュレーター）｜BTC・ETH国内取引所リアルタイム板｜国内暗号資産取引所ナビ',
    description: '日本の暗号資産取引所の実際の取引価格（板情報）をリアルタイムに取得し、成行注文の想定コスト・スリッページ・マーケットインパクトを計算。OKJ・Coincheck・bitFlyer・bitbank・GMOコイン・Binance Japan・BitTrade対応。',
  ...
  'volume-share': {
    ...
    path: '/volume-share',
    name: '出来高シェア',
    title: '国内暗号資産取引所 出来高シェア比較｜国内暗号資産取引所ナビ',
  ...
  derivatives: {
    ...
    path: '/derivatives',
    name: 'デリバティブ',
    title: '国内暗号資産デリバティブ比較｜Crypto CFD・暗号資産FX｜国内暗号資産取引所ナビ',
    description: 'bitFlyer Crypto CFD とGMOコイン暗号資産FXの板、出来高シェア、流動性集中度を比較。デリバティブ市場の変化を自動インサイトで確認できます。',
    ogImagePath: '/ogp/volume-share.png',
    datasetName: '国内暗号資産デリバティブ出来高シェアデータセット',
    datasetDescription: '国内暗号資産デリバティブ市場の銘柄別24時間出来高、取引所別シェア、銘柄内シェアをJPY換算で集計したデータセットです。',
  },
  'sales-spread': {
    pageId: 'sales-spread',
    path: '/sales-spread',
    name: '販売所スプレッド',
    title: '販売所スプレッド比較｜Coincheck・bitFlyer・GMOコイン｜国内暗号資産取引所ナビ',
  ...
  'financial-comparison': {
    ...
    name: '財務比較',
    title: '国内暗号資産取引所 財務比較｜営業収益・利益・純資産・自己資本比率｜国内暗号資産取引所ナビ',
    description: '国内暗号資産取引所の運営会社について、公開財務諸表をもとに営業収益、営業利益、純利益、純資産、総資産、自己資本比率を比較できます。',
    ogImagePath: '/ogp/default.png',
  },
  campaigns: {
    pageId: 'campaigns',
    path: '/campaigns',
    name: 'キャンペーン一覧',
    title: '暗号資産取引所キャンペーン一覧｜PR・紹介リンク確認｜国内暗号資産取引所ナビ',
    description: '国内暗号資産取引所のキャンペーン、特典額、対象者、条件、期間、公式URL、紹介リンクを一覧で確認するページです。申込前に公式条件を確認してください。',
    ogImagePath: '/ogp/default.png',
  },
  research: {
    pageId: 'research',
    path: '/research',
    name: '調べる',
    title: '調べる｜取引所・銘柄プロフィール・初心者ガイド｜国内暗号資産取引所ナビ',
    description: '取引所の信頼性、運営会社・財務情報の確認観点、銘柄プロフィール、特徴・リスク、初心者向けガイドをまとめて調べるページです。',
    ogImagePath: '/ogp/default.png',
  },
};
...
const DATASET_MEASUREMENT_CONFIGS = {
  ...
  derivatives: {
    metricName: 'デリバティブ24時間出来高',
    propertySuffix: 'derivativeQuoteVolume24h',
    unitText: 'JPY',
    valueReferenceName: 'JPY換算デリバティブ出来高',
    description: '国内暗号資産デリバティブ市場の24時間出来高をJPY建てで集計した値',
  },
  'sales-spread': {
    ...
    '@graph': [
      ...
      },
      {
        ...
        potentialAction: {
          ...
          target: {
            '@type': 'EntryPoint',
            urlTemplate: `${origin}/markets?q={search_term_string}`,
          },
...
function webApplicationJsonLd(options = {}) {
  const origin = resolveSiteOrigin(options.origin);
  const page = PAGE_CONFIGS.simulator;
...
function exchangesForDataset(pageId) {
  ...
  return EXCHANGES.filter((exchange) => {
    ...
    if (normalizedPageId === 'derivatives') return marketsForDataset(exchange, normalizedPageId).length > 0;
    return true;
  });
}

function isDerivativeMarket(market) {
  if (!market) return false;
  if (market.marketType === 'derivative') return true;
  return /-CFD-/i.test(String(market.instrumentId || ''));
}

function marketsForDataset(exchange, pageId) {
  const normalizedPageId = normalizePageId(pageId);
  const markets = Array.isArray(exchange && exchange.markets) ? exchange.markets : [];
  return markets
    .filter(market => !market.status || market.status === 'active')
    .filter((market) => {
      const derivativeMarket = isDerivativeMarket(market);
      return normalizedPageId === 'derivatives' ? derivativeMarket : !derivativeMarket;
    });
}
...
function buildDatasetVariableMeasured(pageId) {
  ...
  return exchangesForDataset(normalizedPageId).flatMap((exchange) => {
    return marketsForDataset(exchange, normalizedPageId)
