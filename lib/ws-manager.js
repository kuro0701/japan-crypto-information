...
const { calculateImpact, calculateImpactThresholds, DEFAULT_FEE_RATE } = require('./impact-calculator');
...
class WSManager {
  ...
  handleMessage(ws, msg) {
    ...
    if (msg.type === 'selectExchange' || msg.type === 'selectMarket') {
      ...
      const parsedFeeRate = parseFloat(String(msg.feeRate).replace(/,/g, ''));
      let feeRate = Number.isFinite(parsedFeeRate) ? parsedFeeRate : null;
      if (feeRate == null && msg.feeLevel != null) {
        const fallbackFeeLevel = parseInt(msg.feeLevel, 10) || 1;
        feeRate = ({ 1: 0.0014, 2: 0.0012, 3: 0.0010, 4: 0.0008, 5: 0.0007, 6: 0.0006 }[fallbackFeeLevel]) || DEFAULT_FEE_RATE;
      }
      ...
      const market = this.getMarket(exchangeId, instrumentId);
      if (latestBook) {
        const result = calculateImpact(params.side, params.amount, params.amountType, latestBook, {
  ...
  broadcast(marketKey) {
    ...
    for (const [ws, state] of this.clients) {
      ...
      if (state.simulationParams) {
        ...
        const result = calculateImpact(p.side, p.amount, p.amountType, latestBook, {
          feeRate: p.feeRate,
          market,
        });
        ws.send(JSON.stringify({ type: 'simulation', data: result }));
