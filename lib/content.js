...
function normalizeArticlePath(value, slug) {
  ...
  const pathname = withoutOrigin.split(/[?#]/)[0].replace(/\/+/g, '/');
  if (!/^\/[A-Za-z0-9/_-]+$/.test(pathname)) return fallbackPath;
...
function buildArticle(filePath) {
  ...
  const articlePath = normalizeArticlePath(frontmatter.path, slug);
  ...
  return {
    ...
    date,
    updated,
    author: frontmatter.author || SITE_NAME,
    articleType: frontmatter.articleType || frontmatter.type || '',
    marketTicker: String(frontmatter.marketTicker || frontmatter.ticker || '').trim().toUpperCase(),
    marketInstrumentId: String(frontmatter.marketInstrumentId || frontmatter.instrumentId || '').trim().toUpperCase(),
    category: frontmatter.category || '',
    tags: parseList(frontmatter.tags),
    readMinutes: parsePositiveInteger(frontmatter.readMinutes || frontmatter.readingMinutes),
