...
function createSiteContentService({
  ...
  function renderMarketVisualMetricCards(items) {
    return (items || []).map((item) => {
      const tone = marketToken(item.tone || item.label || 'metric');
      return [
        `<article class="market-visual-metric market-visual-metric--${xmlEscape(tone)}">`,
        `  <span class="market-visual-metric__label">${xmlEscape(item.label)}</span>`,
        `  <strong class="market-visual-metric__value">${xmlEscape(item.value)}</strong>`,
        `  <span class="market-visual-metric__note">${xmlEscape(item.note || '')}</span>`,
        '</article>',
      ].join('\n');
    }).join('\n');
  }

  function renderMarketVisualPillars(items) {
    if (!items || items.length === 0) return '';
    return [
      '<div class="market-visual-pillars">',
      items.map((item, index) => [
        '  <article class="market-visual-pillar">',
        `    <span class="market-visual-pillar__index">${xmlEscape(String(index + 1))}</span>`,
        `    <h4>${xmlEscape(item.title)}</h4>`,
  ...
  function renderMarketVisualTimeline(items) {
    if (!items || items.length === 0) return '';
    return [
      '<article class="market-visual-card market-visual-card--timeline">',
      '  <div class="market-visual-card__head">',
      '    <span>Timeline</span>',
      '    <h4>歴史と制度化の節目</h4>',
      '  </div>',
      '  <ol class="market-visual-timeline">',
      items.map(item => [
        '    <li>',
        `      <time>${xmlEscape(item.date)}</time>`,
        '      <span>',
        `        <strong>${xmlEscape(item.title)}</strong>`,
        `        <small>${xmlEscape(item.body)}</small>`,
        '      </span>',
        '    </li>',
      ].join('\n')).join('\n'),
      '  </ol>',
  ...
  function renderMarketVisualSupply(items) {
    if (!items || items.length === 0) return '';
    return [
      '<article class="market-visual-card market-visual-card--supply">',
      '  <div class="market-visual-card__head">',
      '    <span>Halving</span>',
      '    <h4>半減期と発行ペース</h4>',
      '  </div>',
      '  <div class="market-visual-halving">',
      items.map((item) => {
        const pct = Math.max(2, Math.min(100, Number(item.pct) || 0));
        return [
          '    <div class="market-visual-halving__row">',
          `      <span class="market-visual-halving__period">${xmlEscape(item.period)}</span>`,
          '      <span class="market-visual-halving__track" aria-hidden="true">',
          `        <span style="--halving-width:${xmlEscape(String(pct))}%"></span>`,
          '      </span>',
          `      <strong>${xmlEscape(item.reward)}</strong>`,
          '    </div>',
        ].join('\n');
      }).join('\n'),
      '  </div>',
      '  <p class="market-visual-card__note">バーはローンチ時の50 BTCを100として、ブロック報酬の縮小を相対表示しています。</p>',
      '</article>',
    ].join('\n');
  }

  function renderMarketVisualSnapshot(items) {
    if (!items || items.length === 0) return '';
    return [
      '<article class="market-visual-card market-visual-card--snapshot">',
      '  <div class="market-visual-card__head">',
      '    <span>Snapshot</span>',
      '    <h4>市場データの現在地</h4>',
      '  </div>',
      '  <dl class="market-visual-snapshot">',
      items.map(item => [
        '    <div>',
        `      <dt>${xmlEscape(item.label)}</dt>`,
  ...
  function renderMarketVisualRiskMap(items, scams) {
    if ((!items || items.length === 0) && (!scams || scams.length === 0)) return '';
    const riskTone = (level) => {
      const value = String(level || '').trim();
      if (value === '高') return 'high';
      if (value === '中') return 'medium';
      if (value === '論点') return 'topic';
      return marketToken(value || 'check');
    };
    return [
      '<article class="market-visual-card market-visual-card--risk">',
      '  <div class="market-visual-card__head">',
      '    <span>Risk Map</span>',
      '    <h4>購入前に潰すリスク</h4>',
      '  </div>',
      items && items.length > 0 ? [
        '  <div class="market-visual-risk-grid">',
        items.map((item) => {
          const level = riskTone(item.level);
          return [
            `    <section class="market-visual-risk market-visual-risk--${xmlEscape(level)}">`,
            `      <span>${xmlEscape(item.level || '確認')}</span>`,
      ...
      scams && scams.length > 0 ? [
        '  <div class="market-visual-scam-strip" aria-label="代表的な詐欺パターン">',
        scams.map(item => `    <span>${xmlEscape(item)}</span>`).join('\n'),
        '  </div>',
      ].join('\n') : '',
      '</article>',
    ].filter(Boolean).join('\n');
  ...
  function renderMarketVisualRegulation(items) {
    if (!items || items.length === 0) return '';
    return [
      '<article class="market-visual-card market-visual-card--regulation">',
      '  <div class="market-visual-card__head">',
      '    <span>Regulation</span>',
      '    <h4>主要法域の見方</h4>',
      '  </div>',
      '  <div class="market-visual-regulation">',
      items.map(item => [
        '    <section>',
        `      <strong>${xmlEscape(item.region)}</strong>`,
        `      <p>${xmlEscape(item.stance)}</p>`,
        '    </section>',
      ].join('\n')).join('\n'),
      '  </div>',
      '</article>',
  ...
  function renderMarketVisualResearchBoard(market) {
    const board = getMarketVisualResearchContent(market);
    if (!board) return '';

    return [
      '<div class="market-visual-research-board">',
      '  <article class="market-visual-hero">',
      '    <div class="market-visual-hero__copy">',
      '      <p class="market-insight-card__eyebrow">Research Board</p>',
      `      <h3>${xmlEscape(board.title || `${market.label} リサーチボード`)}</h3>`,
      `      <p>${xmlEscape(board.lead || board.subtitle || '')}</p>`,
      '      <div class="market-visual-source">',
      `        <span>${xmlEscape(board.checkedAt || 'リサーチ確認日未設定')}</span>`,
      `        <strong>${xmlEscape(board.sourceLabel || 'research source')}</strong>`,
      '      </div>',
      '    </div>',
      '    <div class="market-visual-metrics">',
      renderMarketVisualMetricCards(board.keyMetrics),
      '    </div>',
      '  </article>',
      renderMarketVisualPillars(board.pillars),
      '  <div class="market-visual-layout">',
      renderMarketVisualTimeline(board.timeline),
      renderMarketVisualSupply(board.supplySchedule),
      renderMarketVisualSnapshot(board.marketSnapshot),
      renderMarketVisualRiskMap(board.riskMap, board.scamPatterns),
      renderMarketVisualRegulation(board.regulationRows),
      '  </div>',
      board.subtitle ? `  <p class="market-visual-footnote">${xmlEscape(board.subtitle)}</p>` : '',
      '</div>',
    ].filter(Boolean).join('\n');
  }
  ...
  function renderMarketHtml(req, market) {
    ...
    return injectCommonDisclosure(template
      ...
      .replace('<!-- MARKET_VISUAL_RESEARCH_BOARD -->', renderMarketVisualResearchBoard(pageMarket))
