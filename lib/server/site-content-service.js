...
function createSiteContentService({
  ...
  function exchangeHeroScores(exchange) {
    if (exchange.id === 'coincheck') {
      return [
        { label: 'アプリUI', score: 4.7 },
        { label: '板コスト', score: 4.2 },
        { label: '銘柄数', score: 4.1 },
        { label: '信頼性', score: 4.5 },
      ];
    }
    return [
      ...
      { label: '板コスト', score: exchange.boardMarkets.length >= 10 ? 4.2 : 3.4 },
      { label: '銘柄数', score: exchange.coverage.length >= 20 ? 4.4 : 3.5 },
      { label: '信頼性', score: 4.0 },
    ];
  }
  ...
  function renderExchangeHeroReferralAction(exchange) {
    ...
    if (!link) return '';
    const code = exchange.referralCode
      ? `<span class="exchange-hero-referral__code">招待コード <code>${xmlEscape(exchange.referralCode)}</code></span>`
      : '';
    return [
      '<aside class="exchange-hero-referral" aria-label="PRリンク">',
      '  <div>',
      '    <span class="exchange-hero-referral__eyebrow">PR</span>',
      `    <strong>${xmlEscape(exchange.label)} の口座開設・紹介条件を確認</strong>`,
      '    <p>紹介特典、対象者、本人確認、入金・取引条件は申込前に公式画面で確認してください。</p>',
      '  </div>',
      '  <div class="exchange-hero-referral__actions">',
      `    <a class="exchange-hero-referral__button" ${linkAttrsForExchangeAction(link)}><span>PR</span>公式サイトで条件を確認${renderExchangeActionTrackingPixel(link)}</a>`,
      code,
      '  </div>',
      '</aside>',
    ].filter(Boolean).join('\n');
  ...
  function renderExchangeCheckOrder(exchange) {
    ...
    const steps = [
      ...
      },
      {
        ...
        body: referralLink
          ? '紹介リンク経由の特典、手数料、入出金、本人確認、キャンペーン条件を公式画面で見直します。'
          : '手数料、入出金、キャンペーン、本人確認の条件は申込前に一次情報で見直します。',
        link: referralLink || {
