...
function createSiteContentService({
  ...
  const STABLE_BASE_CURRENCIES = new Set(['DAI', 'USD1', 'USDC', 'USDT']);
  const MAJOR_ARTICLE_TICKERS = new Set(['ADA', 'BNB', 'BTC', 'DOGE', 'ETH', 'SOL', 'XRP']);
  ...
  function articleInstrumentId(article) {
    const configured = String(article && article.marketInstrumentId || '').trim().toUpperCase();
    if (configured === 'NONE') return '';
    const explicit = normalizeMarketInstrumentId(configured);
  ...
  function articleCategoryFilter(article) {
    const ticker = articleTicker(article);
    if (STABLE_BASE_CURRENCIES.has(ticker)) return 'stablecoin';
    if (MAJOR_ARTICLE_TICKERS.has(ticker)) return 'major';
    return 'altcoin';
  }

  function normalizedArticleTag(value) {
    return String(value || '')
      .normalize('NFKC')
      .trim()
      .toLowerCase()
  ...
  function articleDisplayTags(article) {
    const ticker = articleTicker(article);
    const tickerKey = normalizedArticleTag(ticker);
    const titleKey = normalizedArticleTag(article && article.title);
    const tags = [];
    const seen = new Set();

    [article && article.category, ...((article && article.tags) || [])].forEach((tag) => {
      const label = String(tag || '').trim();
      const key = normalizedArticleTag(label);
      if (!key || key === tickerKey || seen.has(key)) return;
      if (key.length >= 3 && titleKey.includes(key)) return;
  ...
  function articlePopularityCounts() {
    const counts = new Map();
    if (!analyticsStore || typeof analyticsStore.getReport !== 'function') return counts;

    try {
      const report = analyticsStore.getReport('30d');
      (report.routes || []).forEach((route) => {
        const key = String(route && route.key || '').toLowerCase();
        if (!key.startsWith('/articles/')) return;
        counts.set(key, Number(route.count) || 0);
      });
    } catch (err) {
      console.warn('[Articles] Could not build popularity order:', err.message);
    }
    return counts;
  }

  function renderArticleAssetMark(ticker, className = '') {
    const safeTicker = String(ticker || 'ARTICLE').trim().toUpperCase();
    const classes = [
      'learn-asset-mark',
      safeTicker === 'CANTON' ? 'learn-asset-mark--wide' : '',
      className,
    ].filter(Boolean).join(' ');

    return [
      `<span class="${classes}" aria-hidden="true">`,
      `  <img src="/crypto-icons/${encodeURIComponent(safeTicker)}.svg" alt="" width="24" height="24" decoding="async" data-crypto-logo>`,
      `  <span class="learn-asset-mark__fallback">${xmlEscape(safeTicker.slice(0, 4))}</span>`,
      '</span>',
    ].join('\n');
  }
  ...
  function renderArticleFilterControls(articles) {
    ...
    const categoryFilters = [
      ...
      { value: 'major', label: '主要通貨' },
      { value: 'stablecoin', label: 'ステーブルコイン' },
      { value: 'altcoin', label: 'アルトコイン' },
    ];

    return [
      '<div class="learn-library-toolbar">',
      '  <div class="learn-search-field">',
      '    <label class="learn-search-field__input" for="article-library-search">',
      '      <span class="learn-search-field__icon" aria-hidden="true">検索</span>',
      '      <input id="article-library-search" type="search" data-learn-search placeholder="BTC、税金、リスク、仕組みなど" autocomplete="off">',
      '    </label>',
      '    <button class="learn-command-trigger" type="button" data-article-command-open aria-label="クイック検索を開く（Command K または Control K）">',
      '      <span>クイック検索</span>',
      '      <kbd data-shortcut-label>⌘ K</kbd>',
      '    </button>',
      '  </div>',
      '  <div class="learn-result-tools">',
      '    <p class="learn-result-count" data-learn-result-count aria-live="polite"></p>',
      '    <label class="learn-sort-field">',
      '      <span>並び替え</span>',
      '      <select data-article-sort>',
      '        <option value="latest">最新順</option>',
      '        <option value="popular">人気順（30日）</option>',
      '        <option value="shortest">読了時間が短い順</option>',
      '      </select>',
      '    </label>',
      '  </div>',
      '  <div class="learn-filter-stack">',
      '    <div class="learn-filter-row">',
      '      <span class="learn-filter-row__label">カテゴリ</span>',
      '      <div class="learn-filter-list learn-filter-list--categories" role="group" aria-label="記事カテゴリで絞り込む">',
      ...
      '    </div>',
      '    <div class="learn-filter-row">',
      '      <span class="learn-filter-row__label">銘柄を選ぶ</span>',
      '      <div class="learn-filter-rail">',
      '        <div class="learn-filter-list learn-filter-list--tickers" role="group" aria-label="銘柄で絞り込む">',
      tickerFilters.map(filter => [
        `          <button class="learn-filter-chip learn-filter-chip--ticker" type="button" data-learn-filter="${xmlEscape(filter.value)}" aria-pressed="false">`,
        renderArticleAssetMark(filter.label, 'learn-asset-mark--chip').split('\n').map(line => `            ${line}`).join('\n'),
        `            <span>${xmlEscape(filter.label)}</span>`,
        '          </button>',
      ].join('\n')).join('\n'),
      '        </div>',
      '      </div>',
  ...
  function renderArticleCards(articles, popularityCounts = new Map()) {
    ...
    return articles.map((article) => {
      const ticker = articleTicker(article);
      const filters = [articleCategoryFilter(article), ticker.toLowerCase()].filter(Boolean);
      const tags = articleDisplayTags(article);
      const instrumentId = articleInstrumentId(article);
      const readMinutes = articleReadMinutes(article);
      const popularity = popularityCounts.get(String(article.path || '').toLowerCase()) || 0;
      const searchText = [article.title, article.description, ticker, article.category, ...(article.tags || [])].join(' ');
      return [
        `<a class="learn-card" href="${xmlEscape(article.path)}" data-learn-card data-learn-tags="${xmlEscape(filters.join(' '))}" data-learn-search-text="${xmlEscape(searchText)}" data-article-date="${xmlEscape(article.date)}" data-article-popularity="${xmlEscape(String(popularity))}" data-article-read-minutes="${xmlEscape(String(readMinutes))}">`,
        '  <span class="learn-card__topline">',
        `    <span class="learn-card__icon">${renderArticleAssetMark(ticker)}</span>`,
        '    <span class="learn-card__meta">',
        `      <span class="learn-card__step">${xmlEscape(instrumentId || (ticker ? '国内市場データ対象外' : 'ARTICLE'))}</span>`,
        `      <span class="learn-card__reading-time">読む目安 ${xmlEscape(String(readMinutes))}分</span>`,
        '    </span>',
        '  </span>',
  ...
  function renderArticleCommandPalette(articles) {
    const items = articles.map((article) => {
      const ticker = articleTicker(article);
      const searchText = [article.title, article.description, ticker, article.category, ...(article.tags || [])].join(' ');
      return [
        `<a class="article-command__result" href="${xmlEscape(article.path)}" data-article-command-item data-command-search-text="${xmlEscape(searchText)}">`,
        `  ${renderArticleAssetMark(ticker, 'learn-asset-mark--command')}`,
        '  <span class="article-command__result-copy">',
        `    <strong>${xmlEscape(article.title)}</strong>`,
        `    <span>${xmlEscape(ticker)} ・ 読む目安 ${xmlEscape(String(articleReadMinutes(article)))}分</span>`,
        '  </span>',
        '  <span class="article-command__arrow" aria-hidden="true">↗</span>',
        '</a>',
      ].join('\n');
    }).join('\n');

    return [
      '<div class="article-command" data-article-command hidden>',
      '  <div class="article-command__backdrop" data-article-command-close></div>',
      '  <section class="article-command__dialog" role="dialog" aria-modal="true" aria-labelledby="article-command-title">',
      '    <div class="article-command__header">',
      '      <div>',
      '        <p class="panel-kicker">Quick Search</p>',
      '        <h2 id="article-command-title">記事をすばやく検索</h2>',
      '      </div>',
      '      <button class="article-command__close" type="button" data-article-command-close aria-label="クイック検索を閉じる">Esc</button>',
      '    </div>',
      '    <label class="article-command__search">',
      '      <span aria-hidden="true">検索</span>',
      '      <input type="search" data-article-command-search placeholder="銘柄名、テーマ、キーワードを入力" autocomplete="off">',
      '    </label>',
      '    <div class="article-command__meta">',
      '      <span data-article-command-count></span>',
      '      <span>↑↓で移動 ・ Enterで開く</span>',
      '    </div>',
      '    <div class="article-command__results" data-article-command-results>',
      items,
      '    </div>',
      '    <p class="article-command__empty" data-article-command-empty hidden>一致する記事がありません。</p>',
      '  </section>',
      '</div>',
    ].join('\n');
  }
  ...
  function renderArticlesIndexHtml(req) {
    ...
    const popularityCounts = articlePopularityCounts();
    ...
    return injectCommonDisclosure([
      ...
      '  <link rel="stylesheet" href="/css/style.css?v=articles-ux-3">',
      '</head>',
      ...
      renderArticleCards(articles, popularityCounts),
      '      </div>',
      '      <p class="learn-empty-state" data-learn-empty hidden>条件に合う記事がありません。検索語を短くするか、別の銘柄を選んでください。</p>',
      '      <p class="learn-logo-note">銘柄ロゴは識別目的で掲載し、各権利者に帰属します。CantonはDigital Asset (Switzerland) GmbHの登録商標であり、同社は本サイトと提携・後援・承認していません。</p>',
      '    </section>',
      ...
      renderArticleCommandPalette(articles),
      '  <script src="/js/learn.js?v=articles-ux-1"></script>',
      '</body>',
      '</html>',
  ...
  function articlePrimaryNextAction(article) {
    const instrumentId = articleInstrumentId(article);
    const ticker = articleTicker(article);
    const isMarketArticle = String(article.path || '').startsWith('/articles/');
    if (instrumentId && isMarketArticle && getMarketInfo(instrumentId)) {
      return {
    ...
    if (isMarketArticle) {
      return {
        ...
        kicker: 'Markets',
        title: '国内取扱い銘柄の比較を見る',
        description: '国内取引所で比較できる銘柄の板、販売所スプレッド、出来高を確認します。',
        action: '一覧へ ↗',
      };
    }
  ...
  function articleHeroBrand(article) {
    if (String(article && article.slug || '').toLowerCase() !== 'canton') return '';
    return [
      '<div class="article-hero__brand article-hero__brand--canton" aria-label="Canton Network">',
      '  <img src="/crypto-icons/CANTON.svg" alt="Canton Network" width="1432" height="369">',
      '  <div class="article-hero__brand-meta" aria-label="記事の主要テーマ">',
      '    <span>Selective privacy</span>',
      '    <span>Daml</span>',
  ...
  function articleHeroPresentation(article) {
    const fullTitle = String(article && article.title || '').trim();
    const explicitTitle = String(article && article.heroTitle || '').trim();
    const explicitSubtitle = String(article && article.heroSubtitle || '').trim();
    if (explicitTitle || explicitSubtitle) {
      return {
        title: explicitTitle || fullTitle,
        subtitle: explicitSubtitle,
      };
    }

    const separatorIndex = fullTitle.indexOf('｜');
    if (separatorIndex > 0 && separatorIndex < fullTitle.length - 1) {
      return {
        title: fullTitle.slice(0, separatorIndex).trim(),
        subtitle: fullTitle.slice(separatorIndex + 1).trim(),
      };
    }

    const questionIndex = fullTitle.indexOf('？');
    if (questionIndex > 0 && questionIndex < fullTitle.length - 1) {
      return {
        title: fullTitle.slice(0, questionIndex + 1).trim(),
        subtitle: fullTitle.slice(questionIndex + 1).trim(),
      };
    }
  ...
  function articleHeroTitle(article) {
    const presentation = articleHeroPresentation(article);
    const title = presentation.title;
    if (presentation.subtitle) return xmlEscape(title);

    const groups = [];
    let current = '';

    for (const character of title) {
      if ((character === '｜' || character === '・' || character === ' ') && current) {
        groups.push(current);
        current = character;
        continue;
      }
    ...
    if (groups.length < 2) return xmlEscape(title);

    return groups
      .filter(Boolean)
      .map((group, index) => `<span class="${index === 0 ? 'article-hero__title-phrase' : 'article-hero__title-group'}">${xmlEscape(group)}</span>`)
      .join('');
  }

  function articleHeroSubtitle(article) {
    const subtitle = articleHeroPresentation(article).subtitle;
    if (!subtitle) return '';
    return `<p class="article-hero__subtitle"><span>${xmlEscape(subtitle)}</span></p>`;
  }

  function articleKind(article) {
    return String(article && article.path || '').startsWith('/articles/') ? 'market' : 'guide';
  }

  function renderArticleLeadDisclosure(contentHtml) {
    return [
  ...
  function normalizeArticleLeadDisclosure(article, articleHtml) {
    const html = String(articleHtml || '');
    if (articleKind(article) !== 'market' || /<details\b[^>]*class="[^"]*\barticle-disclosure\b/i.test(html)) {
      return html;
    }

    const replaceLead = (pattern, normalizeContent) => {
      const match = pattern.exec(html);
      if (!match || match.index > 1200 || !match[0].includes('投資助言ではありません')) return '';
      const content = normalizeContent(match[1]);
      return `${html.slice(0, match.index)}${renderArticleLeadDisclosure(content)}${html.slice(match.index + match[0].length)}`;
    };

    const callout = replaceLead(
      /<aside\b[^>]*class="[^"]*\barticle-callout--warning\b[^"]*"[^>]*>([\s\S]*?)<\/aside>/i,
      (content) => {
        const withoutIcon = content.replace(
          /^\s*<span\b[^>]*class="[^"]*\barticle-callout__icon\b[^"]*"[^>]*>[\s\S]*?<\/span>\s*/i,
          ''
        ).trim();
        const wrapper = /^<div>([\s\S]*)<\/div>$/i.exec(withoutIcon);
        return (wrapper ? wrapper[1] : withoutIcon).trim();
      }
    );
    if (callout) return callout;

    const blockquote = replaceLead(
  ...
  function articleSectionHeadings(articleHtml) {
    return Array.from(String(articleHtml || '').matchAll(/<h2(?:\s[^>]*)?>([\s\S]*?)<\/h2>/gi))
      .map((match, index) => ({
        index,
        label: articlePlainText(match[1]),
      }))
      .filter(item => item.label && !/参考(?:資料|文献)|情報源|主要ソース|免責事項/.test(item.label));
  }

  function renderArticleSummaryJumps(items) {
    if (!items.length) return '<p>本文の見出しから確認したいテーマを選べます。</p>';
    return [
      '<div class="article-summary-tabs__jumps" aria-label="本文のセクションへ移動">',
      ...items.map(item => (
        `<button type="button" data-summary-jump="${item.index}">${xmlEscape(item.label)}<span aria-hidden="true">→</span></button>`
      )),
      '</div>',
    ].join('\n');
  }

  function renderArticleSharedDigest(article, articleHtml) {
    const html = String(articleHtml || '');
    if (/data-article-summary-tabs/i.test(html)) return '';

    const headings = articleSectionHeadings(html);
    if (headings.length < 3) return '';

    const isMarket = articleKind(article) === 'market';
    const quickItems = headings.slice(0, 4);
    const focusPattern = isMarket
      ? /価格|市場|流動性|供給|トークノミクス|規制|リスク|評価|投資家/i
      : /比較|選び方|手順|方法|ポイント|注意|リスク|確認/i;
    const focusedItems = headings.filter(item => focusPattern.test(item.label)).slice(0, 4);
    const fallbackFocusedItems = headings.slice(Math.max(0, headings.length - 4));
    const focusLabel = isMarket ? '価格・リスク' : '実践・注意';
    const focusEyebrow = isMarket ? '1-minute risk lens' : 'Action guide';
    const focusCopy = isMarket
      ? '価格・市場・供給・規制・リスクに関係する節だけを選んで確認できます。数値は本文の調査時点とLive Referenceを分けて読んでください。'
      : '比較・手順・注意点に関係する節を先に確認できます。実際の利用前にはリンク先の最新条件も確認してください。';
    const slug = String(article.slug || 'article').replace(/[^a-z0-9_-]/gi, '-') || 'article';
    const readMinutes = Number(articleReadMinutes(article)) || 4;

    return [
      `<section class="article-summary-tabs article-summary-tabs--shared" data-article-summary-tabs data-shared-article-digest aria-label="${xmlEscape(article.title)}の読み方別ナビ">`,
      '  <div class="article-summary-tabs__list" role="tablist" aria-label="読み方を選ぶ">',
      `    <button type="button" role="tab" id="${xmlEscape(slug)}-summary-full-tab" aria-controls="${xmlEscape(slug)}-summary-full" aria-selected="true" data-summary-tab="full">全体像 <small>${xmlEscape(String(readMinutes))}分</small></button>`,
      `    <button type="button" role="tab" id="${xmlEscape(slug)}-summary-quick-tab" aria-controls="${xmlEscape(slug)}-summary-quick" aria-selected="false" tabindex="-1" data-summary-tab="quick">要点ナビ <small>3分</small></button>`,
      `    <button type="button" role="tab" id="${xmlEscape(slug)}-summary-focus-tab" aria-controls="${xmlEscape(slug)}-summary-focus" aria-selected="false" tabindex="-1" data-summary-tab="focus">${focusLabel} <small>1分</small></button>`,
      '  </div>',
      `  <div class="article-summary-tabs__panel" role="tabpanel" id="${xmlEscape(slug)}-summary-full" aria-labelledby="${xmlEscape(slug)}-summary-full-tab" data-summary-panel="full">`,
      '    <span class="article-summary-tabs__eyebrow">Full overview</span>',
      '    <h3>この記事の全体像</h3>',
      `    <p>${xmlEscape(article.description)}</p>`,
      renderArticleSummaryJumps(headings.slice(0, 1)),
      '  </div>',
      `  <div class="article-summary-tabs__panel" role="tabpanel" id="${xmlEscape(slug)}-summary-quick" aria-labelledby="${xmlEscape(slug)}-summary-quick-tab" data-summary-panel="quick" hidden>`,
      '    <span class="article-summary-tabs__eyebrow">3-minute navigator</span>',
      '    <h3>主要セクションから読む</h3>',
      '    <p>本文の主張を作り直さず、実際の見出しから重要な入口をまとめています。</p>',
      renderArticleSummaryJumps(quickItems),
      '  </div>',
      `  <div class="article-summary-tabs__panel" role="tabpanel" id="${xmlEscape(slug)}-summary-focus" aria-labelledby="${xmlEscape(slug)}-summary-focus-tab" data-summary-panel="focus" hidden>`,
      `    <span class="article-summary-tabs__eyebrow">${focusEyebrow}</span>`,
      `    <h3>${focusLabel}を先に確認</h3>`,
      `    <p>${focusCopy}</p>`,
      renderArticleSummaryJumps(focusedItems.length ? focusedItems : fallbackFocusedItems),
      '  </div>',
      '</section>',
    ].join('\n');
  }

  function renderArticleSharedChangelog(article, articleHtml) {
    if (/class="[^"]*\barticle-changelog\b/i.test(String(articleHtml || ''))) return '';
    const publishedDay = String(article.date || '').slice(0, 10);
    const updatedDay = String(article.updated || '').slice(0, 10);
    const changed = publishedDay && updatedDay && publishedDay !== updatedDay;
    const titleId = `${String(article.slug || 'article').replace(/[^a-z0-9_-]/gi, '-')}-changelog-title`;
    const items = [];

    if (changed) {
      items.push([
        `    <li><time datetime="${xmlEscape(updatedDay)}">${xmlEscape(formatArticleDate(article.updated))}</time><div>`,
        '      <span class="article-change-badge article-change-badge--updated">UPDATED</span>',
        '      <strong>記事内容・参照情報を更新</strong>',
        '      <small>更新日の本文を基準に確認できます。</small>',
        '    </div></li>',
      ].join('\n'));
    }
    items.push([
      `    <li><time datetime="${xmlEscape(publishedDay)}">${xmlEscape(formatArticleDate(article.date))}</time><div>`,
      '      <span class="article-change-badge article-change-badge--new">NEW</span>',
      '      <strong>初版を公開</strong>',
      '      <small>記事の公開日です。</small>',
      '    </div></li>',
    ].join('\n'));

    return [
      `<section class="article-changelog article-changelog--shared" data-shared-article-changelog aria-labelledby="${xmlEscape(titleId)}">`,
      '  <div class="article-changelog__header">',
      `    <div><span>Changelog</span><h3 id="${xmlEscape(titleId)}">この記事の更新履歴</h3></div>`,
      '    <small>公開・更新日を表示</small>',
      '  </div>',
  ...
  function injectArticleSharedOverview(article, articleHtml) {
    const html = String(articleHtml || '');
    const modules = [
      renderArticleSharedChangelog(article, html),
      renderArticleSharedDigest(article, html),
    ].filter(Boolean).join('\n');
    if (!modules) return html;

    const leadDisclosure = /^\s*<details\b[^>]*class="[^"]*\barticle-disclosure\b[^"]*"[^>]*>[\s\S]*?<\/details>/i.exec(html);
    if (!leadDisclosure) return `${modules}\n${html}`;
    const insertAt = leadDisclosure.index + leadDisclosure[0].length;
    return `${html.slice(0, insertAt)}\n${modules}${html.slice(insertAt)}`;
  }
  ...
  function renderArticleHtml(req, article) {
    ...
    return injectCommonDisclosure(replaceTemplateSlots(withHead, {
      ...
      ARTICLE_HERO_TITLE: articleHeroTitle(article),
      ARTICLE_HERO_SUBTITLE: articleHeroSubtitle(article),
      ...
      ARTICLE_SLUG: xmlEscape(article.slug || ''),
      ARTICLE_KIND: xmlEscape(articleKind(article)),
      ARTICLE_MARKET_INSTRUMENT_ID: xmlEscape(articleInstrumentId(article) || ''),
      ARTICLE_MARKET_TICKER: xmlEscape(articleTicker(article) || ''),
      ARTICLE_HERO_BRAND: articleHeroBrand(article),
      ARTICLE_PARENT_BREADCRUMB: articleParentBreadcrumb(article),
      ARTICLE_BODY: injectArticleSharedOverview(article, normalizeArticleLeadDisclosure(article, article.html)),
