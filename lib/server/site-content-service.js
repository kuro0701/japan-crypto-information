...
function createSiteContentService({
  ...
  function renderExchangeHeroReferralAction(exchange) {
    ...
    const code = exchange.referralCode
      ? `<span class="exchange-hero-referral__code"><span>招待コード</span><code>${xmlEscape(exchange.referralCode)}</code></span>`
      : '';
    return [
      '<aside class="exchange-hero-referral" aria-label="PR紹介リンク">',
      '  <div class="exchange-hero-referral__copy">',
      '    <span class="exchange-hero-referral__eyebrow">PR / Referral</span>',
      `    <strong>${xmlEscape(exchange.label)}の紹介条件を公式で確認</strong>`,
      '    <p>特典の対象者、本人確認、入金・取引条件は、申込前に公式画面で最新内容を確認してください。</p>',
      '  </div>',
      '  <div class="exchange-hero-referral__actions">',
      `    <a class="exchange-hero-referral__button" ${linkAttrsForExchangeAction(link)}><span>公式</span><strong>紹介条件を見る</strong>${renderExchangeActionTrackingPixel(link)}</a>`,
      code,
      '  </div>',
      '</aside>',
    ].filter(Boolean).join('\n');
