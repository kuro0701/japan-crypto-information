...
function createSiteContentService({
  ...
  const siteNavGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: '取引所選びのチェックリスト', href: '/learn/exchange-checklist' },
        { type: 'separator' },
  ...
  function renderReferralLinkCell(campaign) {
    if (!campaign.affiliateUrl) {
      return '<span class="campaign-link-status">未設定</span>';
    }
  ...
  function renderReferralHighlightCards(campaigns) {
    ...
    return renderCampaignContextCards(highlighted.map(campaign => ({
      ...
      target: campaign.affiliateUrl || campaign.referralSourceUrl || campaign.officialUrl ? '_blank' : null,
      rel: campaign.affiliateUrl ? 'sponsored noopener' : 'noopener',
  ...
  function renderCompareToolCards() {
    const tools = [
      ...
      {
        title: 'デリバティブ',
        description: 'Crypto CFD・暗号資産FXの取引の活発さ（出来高）を見る',
        href: '/derivatives',
      },
      {
        title: '財務比較',
