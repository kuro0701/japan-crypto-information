...
function createSiteContentService({
  ...
  function publicHtmlWithMeta(req, filePath, metaOptions) {
    ...
    return injectCommonDisclosure(withHead);
  }
  ...
  function injectCommonDisclosure(html) {
    if (String(html).includes('class="panel panel--dense common-disclosure"')) return html;
    if (!String(html).includes('</main>')) return html;
    return String(html).replace('</main>', `${renderCommonDisclosure()}\n  </main>`);
  }
  ...
  function renderMarketResearchCards(market) {
    ...
    return [
      ...
      `      <h3 class="market-insight-card__title">${xmlEscape(market.label)} の特徴・リスク</h3>`,
      '    </div>',
      '    <span class="market-insight-card__badge">Beginner Check</span>',
      '  </div>',
      '  <div class="market-insight-card__block">',
      ...
      '  </div>',
      '  <div class="market-insight-card__block">',
      '    <h4 class="market-insight-card__subtitle">初心者が注意すべき点</h4>',
      '    <ol class="market-insight-checklist">',
      (research.risks || []).concat(research.beginnerCautions || []).slice(0, 6).map(item => `      <li>${xmlEscape(item)}</li>`).join('\n'),
      '    </ol>',
      '  </div>',
      '</article>',
    ].join('\n');
  ...
  function renderLearnIndexHtml(req) {
    ...
    return injectCommonDisclosure([
      ...
      '    <section class="panel panel--dense">',
      '      <div class="panel-title-row mb-3">',
      '        <h2 class="panel-title">読んだあとに確認する</h2>',
      '        <span class="panel-kicker">Tools</span>',
      '      </div>',
      '      <div class="home-link-grid">',
      '        <a class="home-link-card" href="/simulator?market=BTC-JPY&side=buy&amountType=jpy&amount=100000">',
      '          <span class="home-link-card__eyebrow">Simulator</span>',
      '          <strong class="home-link-card__title">BTC 10万円買いの板シミュレーション</strong>',
      '          <span class="home-link-card__description">スリッページや成行注文のリスクを、実際の板データで確認できます。</span>',
      '        </a>',
      '        <a class="home-link-card" href="/sales-spread?instrumentId=BTC-JPY">',
      '          <span class="home-link-card__eyebrow">Spread</span>',
      '          <strong class="home-link-card__title">BTC/JPY の販売所スプレッド</strong>',
      '          <span class="home-link-card__description">販売所の買値と売値の差を、銘柄別・取引所別に比較できます。</span>',
      '        </a>',
      '      </div>',
      '    </section>',
      '  </main>',
  ...
  function renderResearchExchangeCards(exchanges) {
    return exchanges.map((exchange) => [
      ...
      `  <span class="market-context-card__description">${xmlEscape(`${exchange.marketCount}銘柄 / 既定 taker ${exchange.feeLabel} / 運営会社・信頼性・長期利用の確認へ`)}</span>`,
      '  <span class="market-context-card__cta">取引所詳細を見る</span>',
      '</a>',
    ].join('\n')).join('\n');
  }
  ...
  function renderResearchMarketCards(markets) {
    return markets.map((market) => {
      ...
      return [
        ...
        `  <span class="market-context-card__description">${xmlEscape(`${research.category} / ${research.plainUse}`)}</span>`,
        '  <span class="market-context-card__cta">用途・リスクを見る</span>',
        '</a>',
      ].join('\n');
    }).join('\n');
  }
  ...
  function renderResearchHtml(req) {
    ...
    return injectCommonDisclosure([
      ...
      '          <span class="home-role-card__number">2</span>',
      '          <div>',
      ...
      '      <div class="panel-title-row mb-4">',
      '        <div>',
      '          <h2 class="panel-title">取引所を調べる</h2>',
      '          <p class="text-xs text-gray-500 mt-1">取引所詳細ページで、信頼性、運営会社、取扱銘柄、通常時コストを確認します</p>',
      '        </div>',
      ...
      '      <div class="panel-title-row mb-4">',
      '        <div>',
      '          <h2 class="panel-title">銘柄を調べる</h2>',
      '          <p class="text-xs text-gray-500 mt-1">銘柄プロフィールで、用途、特徴、リスク、対応取引所を確認します</p>',
      '        </div>',
      ...
      '      <div class="panel-title-row mb-4">',
      '        <div>',
      '          <h2 class="panel-title">初心者向けガイド</h2>',
      '          <p class="text-xs text-gray-500 mt-1">販売所、取引所、成行注文、手数料、運営会社の確認観点を整理します</p>',
      '        </div>',
  ...
  function renderMarketHtml(req, market) {
    ...
    const title = `${pageMarket.label} 用途・特徴・リスクと取引所比較｜${SITE_NAME}`;
    const description = `${pageMarket.label} の用途、特徴、初心者が注意すべきリスク、国内暗号資産取引所の板情報、出来高シェア、販売所スプレッド、取引所別の成行コストを1ページで確認できます。`;
    ...
    return injectCommonDisclosure(template
      ...
      .replace('<!-- MARKET_RESEARCH_CARDS -->', renderMarketResearchCards(pageMarket))
