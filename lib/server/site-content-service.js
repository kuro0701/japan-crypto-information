...
function createSiteContentService({
  ...
  function renderCoinArticleTextSection(section) {
    ...
    return [
      `<section id="${xmlEscape(id)}" class="market-coin-article__section">`,
  ...
  function renderCoinArticleContents(items) {
    ...
    return [
      ...
      '  <strong>目次</strong>',
      '  <div>',
      items.map(item => `    <a href="${xmlEscape(item.href)}">${xmlEscape(item.label)}</a>`).join('\n'),
      '  </div>',
      '</nav>',
    ].join('\n');
  }
  ...
  function renderCoinArticleFaq(items) {
    if (!Array.isArray(items) || items.length === 0) return '';
    return [
      '<section id="coin-article-faq" class="market-coin-article__faq">',
      '  <div class="market-coin-article__block-head">',
      '    <span>FAQ</span>',
      '    <h4>ビットコインFAQ</h4>',
      '  </div>',
