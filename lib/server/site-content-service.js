...
function createSiteContentService({
  ...
  function buildVolumeShareCampaignPayload() {
    ...
    listCampaigns().forEach((campaign) => {
      ...
      exchanges[campaign.exchangeId] = {
        ...
        lastChecked: campaign.lastChecked,
  ...
  function renderVolumeShareCampaignDataScript() {
    return `<script>window.VolumeShareCampaigns=${jsonForInlineScript(buildVolumeShareCampaignPayload())};</script>`;
  }
  ...
  function renderVolumeShareHtml(req) {
    ...
    const withHead = html
      ...
      .replace('<!-- VOLUME_SHARE_CAMPAIGN_DATA -->', renderVolumeShareCampaignDataScript());
  ...
  function renderFinancialHighlightCards(model) {
    return model.highlights.map((item) => {
      ...
      return [
        ...
        renderFinancialSparkline(company, item.key),
        '</article>',
      ].join('\n');
    }).join('\n');
  }
  ...
  function renderFinancialComparisonHtml(req) {
    ...
    return injectCommonDisclosure([
      ...
      '  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>',
      '  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Noto+Sans+JP:wght@400;500;700&display=swap" rel="stylesheet">',
      '  <link rel="stylesheet" href="/css/style.css?v=ui-24">',
      '</head>',
      ...
      '          <p class="text-xs text-gray-500 mt-1">公開資料で確認できる範囲で、顧客預かり暗号資産と自己保有暗号資産を銘柄別に並べます。未開示の内訳は推定せず、そのまま表示します。</p>',
      '        </div>',
      ...
      '      <div id="financial-asset-balance-table" class="financial-asset-table-wrap overflow-auto scrollbar-thin"></div>',
      ...
      '  <script src="/js/financial-comparison.js?v=6"></script>',
      '</body>',
      '</html>',
