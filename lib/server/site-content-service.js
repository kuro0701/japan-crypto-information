...
function createSiteContentService({
  ...
  function renderExchangeConclusionCard(exchange) {
    ...
    return [
      ...
      `        <span>${xmlEscape(`${defaultMarketLabel}を起点に、板取引対応 ${formatCount(boardCount)}件と販売所対応 ${formatCount(salesCount)}件を分けて見られます。`)}</span>`,
      '      </div>',
      '      <div class="exchange-hero-point">',
      ...
      '        <strong>販売所スプレッドと10万円買いの実質コスト</strong>',
      `        <span>${xmlEscape(spreadSummary)} 取引所形式では手数料、板スプレッド、スリッページを合わせて見ます。</span>`,
      '      </div>',
      '  </div>',
      '</div>',
    ].join('\n');
