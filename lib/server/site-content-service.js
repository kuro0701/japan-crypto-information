...
function createSiteContentService({
  ...
  function renderExchangeCostSimulator(exchange, options = {}) {
    ...
    return [
      `<article class="${classes}" data-exchange-cost-simulator data-exchange-id="${xmlEscape(exchange.id)}" data-instrument-id="${xmlEscape(exchange.defaultMarket.instrumentId)}" data-default-amount="${xmlEscape(String(defaultAmount))}">`,
  ...
  function renderExchangeInlineCta(exchange, options = {}) {
    ...
    const body = fallbackText(options.body, '手数料、対象サービス、キャンペーン条件は変わる場合があります。申込前に公式条件を確認してください。');
  ...
  function renderExchangeReferralCopy(exchange) {
    if (!exchange.referralCode) return '';
    return [
      '<section class="exchange-referral-copy-panel" aria-label="紹介コードコピー">',
      '  <div>',
      '    <span class="exchange-referral-copy-panel__eyebrow">Referral Code</span>',
      `    <strong>${xmlEscape(exchange.label)} の紹介コード</strong>`,
      '  </div>',
      '  <div class="exchange-referral-copy-panel__code">',
      `    <code>${xmlEscape(exchange.referralCode)}</code>`,
      `    <button type="button" data-exchange-copy-value="${xmlEscape(exchange.referralCode)}" data-exchange-copy-label="紹介コード">コピー</button>`,
      '  </div>',
      '  <span class="exchange-copy-feedback" data-exchange-copy-feedback aria-live="polite"></span>',
      '</section>',
    ].join('\n');
  }
  ...
  function renderExchangeRelatedLinks(exchange, aboutArticle) {
    ...
    return [
      ...
      renderExchangeReferralCopy(exchange),
    ].filter(Boolean).join('\n');
  }
