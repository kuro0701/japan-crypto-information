...
function createSiteContentService({
  ...
  function renderMarketProfileCards(market) {
    ...
    return [
      ...
      `      <h3 class="market-insight-card__title">${xmlEscape(market.label)} の特徴</h3>`,
      '    </div>',
      ...
      '  </div>',
      '  <div class="market-insight-card__block">',
      '    <h4 class="market-insight-card__subtitle">国内取引所で見るポイント</h4>',
      '    <ol class="market-insight-checklist">',
      localNotes.map(item => `      <li>${xmlEscape(item)}</li>`).join('\n'),
      '    </ol>',
      '  </div>',
      '</article>',
    ].join('\n');
  ...
  function buildMarketPreTradeChecklist(market) {
    ...
    return {
      checks: [
        `${market.label} を販売所で買う場合は、買値・売値の差を確認する`,
        '取引所形式で売買できる取引所があるか確認する',
        '実際の注文金額で取引所別コスト比較を試す',
        'Impact が高い場合は注文サイズ、注文方法、取引所を見直す',
        '送金予定がある場合は対応ネットワーク、最小出金額、出金手数料を公式画面で確認する',
      ],
  ...
  function renderMarketPreTradeCheckCards(market) {
    ...
    return [
      ...
      '  </div>',
      '  <p class="market-insight-card__lead">購入判断の前に、表示価格だけでなく実効コストと注文条件を確認します。</p>',
      '  <ol class="market-insight-checklist market-insight-checklist--strong">',
      checklist.checks.map(item => `    <li>${xmlEscape(item)}</li>`).join('\n'),
      '  </ol>',
      '</article>',
