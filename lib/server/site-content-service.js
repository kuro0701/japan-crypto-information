...
function createSiteContentService({
  ...
  function renderMarketConclusionCard(market) {
    ...
    const statValueHtml = (value, meta, options = {}) => [
      `<span class="market-conclusion-stat__value${options.signal ? ' market-conclusion-stat__value--signal' : ''}">${options.html ? value : xmlEscape(value)}</span>`,
      `<span class="market-conclusion-stat__meta">${xmlEscape(meta)}</span>`,
    ].join('');
    const topVolumeLabel = market.topVolumeExchangeLabel || 'データ待ち';
    const bestSpreadLabel = market.bestSpreadExchangeLabel || 'データ待ち';
    const dataPoints = [
      {
        label: '取扱',
        valueHtml: statValueHtml(`${formatCount(market.exchangeCount)}社`, '国内現物対応'),
      },
      {
        label: '出来高首位',
        valueHtml: statValueHtml(
          market.topVolumeExchangeLabel
            ? `<span class="market-signal-badge">${xmlEscape(topVolumeLabel)}</span>`
            : xmlEscape(topVolumeLabel),
          '24h売買代金ベース',
          { html: true, signal: Boolean(market.topVolumeExchangeLabel) }
        ),
      },
      {
        label: '販売所最小',
        valueHtml: statValueHtml(
          market.bestSpreadExchangeLabel
            ? `<span class="market-signal-badge">${xmlEscape(bestSpreadLabel)}</span>${market.bestSpreadPct != null ? ` <span>${xmlEscape(formatPct(market.bestSpreadPct))}</span>` : ''}`
            : xmlEscape(bestSpreadLabel),
          '販売所スプレッド',
          { html: true, signal: Boolean(market.bestSpreadExchangeLabel) }
        ),
      },
    ];
  ...
  function renderMarketPreTradeCheckCards(market) {
    ...
    return [
      ...
      '  </ul>',
      '  <div class="market-pretrade-ready" data-pretrade-ready hidden><span aria-hidden="true"></span><strong>準備万端です!</strong></div>',
      '</article>',
  ...
  function renderMarketSupportedExchangeLinks(market) {
    return market.supportedExchanges.map((exchange) => [
      ...
      `  <span class="market-context-card__description">既定 <span class="market-term" data-term-key="taker" tabindex="0">taker</span> ${xmlEscape(exchange.feeLabel)} / ${xmlEscape(`${exchange.marketCount}銘柄`)} / 入出金 ${xmlEscape(exchange.fundingLabel || '公式条件確認')}。手数料、取扱銘柄、運営会社・財務情報を確認できます。</span>`,
