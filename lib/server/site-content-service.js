...
function createSiteContentService({
  ...
  function renderFinancialMetricButtons(metrics) {
    return metrics.map((metric, index) => [
      `<button class="btn-tab financial-metric-tab${index === 0 ? ' is-active' : ''}" type="button" data-financial-metric="${xmlEscape(metric.key)}" aria-pressed="${index === 0 ? 'true' : 'false'}" title="${xmlEscape(metric.help || metric.description || metric.label)}">`,
  ...
  function renderFinancialComparisonHtml(req) {
    ...
    return injectCommonDisclosure([
      ...
      '  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>',
      '  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Noto+Sans+JP:wght@400;500;700&display=swap" rel="stylesheet">',
      '  <link rel="stylesheet" href="/css/style.css?v=ui-23">',
      '</head>',
      ...
      '  <script src="/js/financial-comparison.js?v=3"></script>',
      '</body>',
      '</html>',
