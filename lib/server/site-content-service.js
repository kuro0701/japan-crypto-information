...
function createSiteContentService({
  ...
  function renderExchangeHeroReferralAction(exchange) {
    ...
    const code = exchange.referralCode
      ? [
        `<button class="exchange-referral-code-button exchange-hero-referral__code" type="button" data-exchange-copy-value="${xmlEscape(exchange.referralCode)}" data-exchange-copy-label="招待コード" aria-label="招待コード ${xmlEscape(exchange.referralCode)} をコピー">`,
        '  <span>招待コード</span>',
        `  <code>${xmlEscape(exchange.referralCode)}</code>`,
        '  <b data-exchange-copy-button-label>Tap to copy</b>',
        '</button>',
      ].join('\n')
      : '';
    return [
      ...
      '    <p>コードをコピーして公式の招待ページへ進み、対象者・本人確認・入金や取引条件を申込前に確認してください。</p>',
      '    <ol class="exchange-referral-steps" aria-label="紹介特典の流れ">',
      '      <li><span>1</span><strong>コードコピー</strong></li>',
  ...
  function renderExchangeReferralCopy(exchange) {
    ...
    return [
      ...
      '  <div>',
      '    <span class="exchange-referral-copy-panel__eyebrow">招待プログラム</span>',
      `    <strong>${xmlEscape(exchange.label)} の招待コードを再コピー</strong>`,
      '  </div>',
      '  <div class="exchange-referral-copy-panel__code">',
      `    <button class="exchange-referral-code-button" type="button" data-exchange-copy-value="${xmlEscape(exchange.referralCode)}" data-exchange-copy-label="招待コード" aria-label="招待コード ${xmlEscape(exchange.referralCode)} をコピー">`,
      `      <code>${xmlEscape(exchange.referralCode)}</code>`,
      '      <b data-exchange-copy-button-label>Tap to copy</b>',
      '    </button>',
      '  </div>',
