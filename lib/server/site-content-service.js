...
function createSiteContentService({
  ...
  function renderExchangeConclusionCard(exchange) {
    ...
    const spreadText = exchange.spread.marketCount > 0
      ? `販売所スプレッドは ${exchange.spread.marketCount}銘柄を追跡中で、${exchange.spread.summary}`
      : '販売所スプレッドはデータ待ちです。販売所を使う場合は公式画面の買値・売値差を確認してください。';
    const volumeText = exchange.volume.exchangeShare
      ? `出来高シェアは ${formatPct(exchange.volume.exchangeShare.sharePct, 1)} で、板の厚みを見る候補になります。`
      : '出来高シェアはデータ待ちです。主要銘柄から板の厚みを確認してください。';
    const verdict = `${exchange.label} は、取扱銘柄・通常時コスト・流動性を同じ画面で確認してから使う候補です。`;
    ...
    return [
      ...
      `        <li>${xmlEscape(`取扱銘柄は ${formatCount(exchange.coverage.length)}件。板取引 ${formatCount(boardCount)}件 / 販売所データ ${formatCount(salesCount)}件を分けて見ます。`)}</li>`,
      `        <li>${xmlEscape(`既定 taker 手数料は ${exchange.feeLabel}。会員区分や銘柄ごとの差は公式条件で確認します。`)}</li>`,
      `        <li>${xmlEscape(spreadText)}</li>`,
      `        <li>${xmlEscape(volumeText)}</li>`,
      '      </ul>',
      '      <dl class="market-conclusion-facts exchange-conclusion-facts">',
      '        <div>',
      '          <dt>既定 taker 手数料</dt>',
      `          <dd>${xmlEscape(exchange.feeLabel)}</dd>`,
      '        </div>',
      '        <div>',
      '          <dt>取扱銘柄数</dt>',
      `          <dd>${xmlEscape(`${formatCount(exchange.coverage.length)}件`)}</dd>`,
      '        </div>',
      '        <div>',
      '          <dt>基準銘柄</dt>',
  ...
  function renderExchangeFeeSpreadCards(exchange) {
    const cards = [
      {
        title: '取引所手数料',
        description: `${exchange.feeLabel} を比較の基準にしています。maker/taker、会員ランク、銘柄、キャンペーン適用で変わる場合があります。`,
        badge: 'Fee',
      },
      {
        title: '販売所スプレッド',
      ...
      },
      {
        title: '狭い / 広い銘柄の目安',
        description: exchange.spread.marketCount > 0
          ? `直近データでは、狭めの候補は ${formatExchangeSpreadMarket(exchange.spread.tightest)}、広めの候補は ${formatExchangeSpreadMarket(exchange.spread.widest)} です。`
          : 'スプレッドデータが取得できた銘柄から、狭い候補と広い候補を表示します。',
        badge: 'Range',
      },
      {
        title: '入出金コスト',
        description: `${fundingSummaryLabel(exchange.funding)}。銀行、銘柄、ネットワーク、混雑状況で条件が変わるため、出金前に公式手数料表を確認してください。`,
        badge: 'Funding',
      },
    ];
  ...
  function renderExchangeOrderbookVolumeCards(exchange) {
    ...
    const cards = [
      {
        title: '板取引対応銘柄',
        description: 'このサイトで板シミュレーターに接続している銘柄です。大きめの注文では、手数料だけでなく板の厚みを確認します。',
        markets: exchange.boardMarkets.slice(0, 12),
        emptyMessage: '板取引対応銘柄を準備中です。',
        badge: `${formatCount(exchange.boardMarkets.length)}件`,
      },
      {
        title: '出来高シェア',
      ...
      {
        title: '板の厚い銘柄の参考',
        description: exchange.thickMarkets.length > 0
          ? '24h 出来高が大きい銘柄を並べています。まずここから板シミュレーターで注文サイズに対する影響を見ます。'
          : '出来高データを取得できた銘柄から、板の厚みを確認する候補を表示します。',
      ...
      {
        title: 'データ取得元',
        description: `${exchange.dataSourceLabel || '公開API / WebSocket'} を利用して板・出来高の参考データを表示しています。取得失敗や配信遅延がある場合は公式画面を優先してください。`,
        badge: 'Source',
      },
    ];
  ...
  function renderExchangeCompanyCards(exchange) {
    const cards = [
      {
        eyebrow: 'Trust',
        title: 'この取引所は信頼できる？',
        lead: `${exchange.label} の信頼性は、知名度だけでなく登録状況、顧客資産管理、障害時のお知らせ、通常時コストを分けて確認します。`,
        checks: [
          '金融庁・財務局の暗号資産交換業者登録、公式会社概要、利用規約を一次情報で確認する',
          '顧客資産の分別管理、セキュリティ方針、出金停止時の対応、補償方針を確認する',
      ...
      },
      {
        eyebrow: 'Long Term',
        title: '長く使うなら見る点',
        lead: `${exchange.label} を長く使う候補にする場合は、キャンペーンよりも通常時の使いやすさ、流動性、入出金条件を優先して見ます。`,
        checks: [
    ...
    return cards.map((card) => [
      ...
      `    <span>${xmlEscape(`${formatCount(exchange.coverage.length)}銘柄`)}</span>`,
  ...
  function renderExchangeOverviewCards(exchange) {
    ...
    const cards = [
      ...
      {
        term: '入出金対応',
        description: `${fundingSummaryLabel(exchange.funding)}。${exchange.funding && exchange.funding.note ? exchange.funding.note : '銀行・銘柄・ネットワーク別の条件は公式確認してください。'}`,
      },
    ];
