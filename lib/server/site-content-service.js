...
function createSiteContentService({
  ...
  function renderMarketStatusBadges(market) {
    ...
    return badges.map(badge => (
      `<span class="market-status-badge market-status-badge--${xmlEscape(badge.tone)} article-term" data-term-key="${xmlEscape(badge.termKey)}" tabindex="0">${xmlEscape(badge.label)}</span>`
    )).join('');
  }
  ...
  function renderMarketIndexCards(markets) {
    ...
    return markets.map((market) => {
      ...
      const bestSpreadText = market.bestSpreadExchangeLabel
        ? `スプレッド最小：${market.bestSpreadExchangeLabel}${market.bestSpreadPct != null ? ` ${formatPct(market.bestSpreadPct)}` : ''}`
        : (market.marketType === 'derivative' ? 'デリバティブ板' : '販売所データなし');
      const changeText = market.change24hPct != null
        ? `${formatSignedPct(market.change24hPct)}${market.change24hExchangeLabel ? ` / ${market.change24hExchangeLabel}` : ''}`
        : '24h変化データ待ち';
      ...
      return [
        ...
        '    <span class="market-index-card__trend">',
        `      <span class="market-index-card__trend-copy"><span class="market-index-card__trend-label">24h変化</span><span class="market-index-card__trend-value market-trend market-trend--${xmlEscape(trendTone)}" data-market-change-text>${xmlEscape(changeText)}</span></span>`,
        `      ${renderSparkline(market.instrumentId, market.change24hPct)}`,
        '    </span>',
        '    <span class="market-index-card__metrics">',
        `      <span class="market-index-card__metric" data-market-top-volume>${xmlEscape(topVolumeText)}</span>`,
        `      <span class="market-index-card__metric" data-market-best-spread>${xmlEscape(bestSpreadText)}</span>`,
        '    </span>',
  ...
  function renderMarketIndexTableRows(markets) {
    ...
    return markets.map((market) => {
      ...
      return [
        ...
        `  <td data-label="出来高トップ" data-sort-value="${marketSortValue(market.instrumentTotalQuoteVolume, '0')}" data-market-top-volume>${xmlEscape(topVolumeText)}</td>`,
        `  <td data-label="最小スプレッド" data-sort-value="${marketSortValue(market.bestSpreadPct)}" data-market-best-spread>${xmlEscape(bestSpreadText)}</td>`,
        `  <td data-label="24h変化" data-sort-value="${marketSortValue(market.change24hPct)}"><span class="market-trend market-trend--${xmlEscape(trendTone)}" data-market-change-text>${xmlEscape(changeText)}</span></td>`,
        '  <td data-label="操作">',
  ...
  function renderExchangeRelatedLinks(exchange, aboutArticle) {
    ...
    function renderRelatedLink(link) {
      ...
      const externalAttrs = isExternal
        ? ` target="_blank" rel="${xmlEscape(link.rel || 'noopener noreferrer')}"`
        : '';
      return [
