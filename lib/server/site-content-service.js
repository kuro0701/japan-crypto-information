...
function createSiteContentService({
  ...
  const sitemapPages = [
    ...
    },
    {
      ...
      lastmod: fileLastmod(path.join(publicDir, 'referral-benefits.html')),
      priority: '0.7',
    },
    {
      path: '/learn',
  ...
  function volumeCampaignTag(campaign) {
    if (campaign.referralSummary || campaign.referralCode) return '招待特典あり';
  ...
  const siteNavGroups = [
    ...
    },
    {
      ...
      label: 'キャンペーン',
      items: [
        { label: '開催中のキャンペーン総合', href: '/campaigns' },
        { label: '友達招待コード・特典まとめ', href: '/campaigns/referrals' },
        { label: 'bitFlyer 招待プログラム', href: '/campaigns/bitflyer' },
        { label: 'Coincheck お得な始め方', href: '/campaigns/coincheck' },
        { label: 'GMOコイン キャンペーン情報', href: '/campaigns/gmo-coin' },
      ],
    },
  ];
  ...
  function navOptionsForPage(pageId) {
    const byPageId = {
      ...
      campaigns: { activeSection: 'campaigns', activePath: '/campaigns' },
    };
  ...
  function navSectionForPath(value) {
    ...
    if (navPath === '/campaigns' || navPath.startsWith('/campaigns/') || navPath === '/referral-benefits') return 'campaigns';
  ...
  function renderAssetMark(market, sizeClass = '') {
    ...
    const label = String(base).slice(0, 4).toUpperCase();
  ...
  function referralBenefitsStructuredData(campaigns, origin, description) {
    const pageUrl = siteUrl(origin, '/campaigns/referrals');
    return [
      {
        '@context': 'https://schema.org',
        '@type': 'WebPage',
        name: '国内暗号資産取引所 紹介特典比較',
        url: pageUrl,
        description,
        inLanguage: 'ja-JP',
        isPartOf: {
      ...
      },
      {
        ...
        itemListElement: campaigns.map((campaign, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${campaign.exchangeName} ${campaign.referralSummary || '紹介特典公式確認'}`,
          url: siteUrl(origin, campaign.path || campaignPath(campaign)),
        })),
      },
    ];
  ...
  function campaignAffiliateTargetAttr(campaign) {
    if (campaign.affiliateTarget === null) return '';
    return ` target="${xmlEscape(campaign.affiliateTarget || '_blank')}"`;
  }

  function campaignAffiliateRel(campaign) {
    return campaign.affiliateRel || 'sponsored noopener';
  }

  function campaignAffiliateReferrerPolicyAttr(campaign) {
    return campaign.affiliateReferrerPolicy
      ? ` referrerpolicy="${xmlEscape(campaign.affiliateReferrerPolicy)}"`
      : '';
  }

  function renderCampaignTrackingPixel(campaign) {
    if (!campaign.trackingPixelUrl) return '';
    return `<img src="${xmlEscape(campaign.trackingPixelUrl)}" width="1" height="1" border="0" alt="">`;
  }

  function renderCampaignAffiliateAnchor(campaign, label, className) {
    if (!campaign.affiliateUrl) {
      return null;
    }

    return [
      `<a class="${xmlEscape(className)}" href="${xmlEscape(campaign.affiliateUrl)}"${campaignAffiliateTargetAttr(campaign)} rel="${xmlEscape(campaignAffiliateRel(campaign))}"${campaignAffiliateReferrerPolicyAttr(campaign)}>`,
      `  ${xmlEscape(label)}${renderCampaignTrackingPixel(campaign)}`,
      '</a>',
    ].join('\n');
  }

  function renderCampaignActionLink(campaign) {
    return renderCampaignAffiliateAnchor(campaign, '紹介リンクを開く', 'campaign-affiliate-link')
      || '<span class="campaign-link-status">紹介リンク未設定</span>';
  }
  ...
  function renderCampaignCards(campaigns) {
    return campaigns.map((campaign) => [
      ...
      renderCampaignAffiliateAnchor(campaign, '紹介リンク', 'btn btn-primary px-4 py-3 rounded-lg font-bold text-sm')
  ...
  function campaignHasReferralBenefit(campaign) {
    return Boolean(campaign.referralSummary || campaign.referrerBenefit || campaign.inviteeBenefit);
  }

  function referralFallback(value) {
    return value || '公式確認';
  }

  function renderReferralLinkCell(campaign) {
    return renderCampaignAffiliateAnchor(campaign, '紹介リンクを開く', 'campaign-affiliate-link')
      || '<span class="campaign-link-status">未設定</span>';
  }

  function renderReferralSourceCell(campaign) {
    const href = campaign.referralSourceUrl || campaign.officialUrl;
    const label = campaign.referralSourceUrl ? '紹介条件' : '公式URL';
    return `<a class="table-link" href="${xmlEscape(href)}" target="_blank" rel="noopener">${xmlEscape(label)}</a>`;
  }

  function renderReferralBenefitRows(campaigns) {
    return campaigns.map((campaign) => [
      '<tr>',
      `  <td data-label="取引所名">${xmlEscape(campaign.exchangeName)}</td>`,
      `  <td data-label="招待された方">${xmlEscape(referralFallback(campaign.inviteeBenefit))}</td>`,
      `  <td data-label="招待した方">${xmlEscape(referralFallback(campaign.referrerBenefit))}</td>`,
      `  <td data-label="条件">${xmlEscape(referralFallback(campaign.referralTrigger))}</td>`,
      `  <td data-label="招待コード">${xmlEscape(campaign.referralCode || '未掲載')}</td>`,
      `  <td data-label="紹介リンク">${renderReferralLinkCell(campaign)}</td>`,
      `  <td data-label="公式確認">${renderReferralSourceCell(campaign)}</td>`,
      `  <td data-label="詳細"><a class="table-link" href="${xmlEscape(campaign.path || campaignPath(campaign))}">詳細</a></td>`,
      '</tr>',
    ].join('\n')).join('\n');
  }

  function renderReferralHighlightCards(campaigns) {
    const highlighted = campaigns.filter(campaignHasReferralBenefit);
    if (highlighted.length === 0) {
      return renderCampaignContextCards([{
        eyebrow: 'Referral',
        title: '紹介特典額は公式確認',
        description: '紹介特典額を掲載する場合は、公式URLとPR表記を併記して追加します。',
        href: '/campaigns',
        cta: 'キャンペーン一覧へ',
      }]);
    }

    return renderCampaignContextCards(highlighted.map(campaign => ({
      eyebrow: campaign.exchangeName,
      title: campaign.referralSummary || campaign.benefit,
      description: `${campaign.exchangeName} の紹介条件を公式情報とあわせて確認します。`,
      href: campaign.affiliateUrl || campaign.referralSourceUrl || campaign.officialUrl,
      target: campaign.affiliateUrl
        ? (campaign.affiliateTarget === null ? null : (campaign.affiliateTarget || '_blank'))
        : (campaign.referralSourceUrl || campaign.officialUrl ? '_blank' : null),
      rel: campaign.affiliateUrl ? campaignAffiliateRel(campaign) : 'noopener',
      referrerPolicy: campaign.affiliateUrl ? campaign.affiliateReferrerPolicy : null,
      trackingPixelUrl: campaign.affiliateUrl ? campaign.trackingPixelUrl : null,
      cta: campaign.affiliateUrl ? '紹介リンクを開く' : '公式で確認',
    })));
  }

  function renderReferralBenefitCards(campaigns) {
    return campaigns.map((campaign) => {
      const hasBenefit = campaignHasReferralBenefit(campaign);
      return [
        '<article class="campaign-card">',
        '  <div class="campaign-card__header">',
        '    <div>',
        `      <span class="campaign-card__exchange">${xmlEscape(campaign.exchangeName)}</span>`,
        `      <h3 class="campaign-card__title">${xmlEscape(hasBenefit ? campaign.referralSummary : '紹介特典は公式確認')}</h3>`,
        '    </div>',
        `    <span class="decision-summary-badge">${xmlEscape(hasBenefit ? '特典額あり' : '公式確認')}</span>`,
        '  </div>',
        '  <dl class="campaign-card__facts">',
        `    <div><dt>招待された方</dt><dd>${xmlEscape(referralFallback(campaign.inviteeBenefit))}</dd></div>`,
        `    <div><dt>招待した方</dt><dd>${xmlEscape(referralFallback(campaign.referrerBenefit))}</dd></div>`,
        `    <div><dt>招待コード</dt><dd>${xmlEscape(campaign.referralCode || '未掲載')}</dd></div>`,
        `    <div><dt>条件</dt><dd>${xmlEscape(referralFallback(campaign.referralTrigger))}</dd></div>`,
        '  </dl>',
        '  <div class="campaign-card__actions">',
        `    <a class="btn btn-secondary px-4 py-3 rounded-lg font-bold text-sm" href="${xmlEscape(campaign.path || campaignPath(campaign))}">詳細ページ</a>`,
        `    <a class="btn btn-secondary px-4 py-3 rounded-lg font-bold text-sm" href="${xmlEscape(campaign.referralSourceUrl || campaign.officialUrl)}" target="_blank" rel="noopener">公式確認</a>`,
        renderCampaignAffiliateAnchor(campaign, '紹介リンク', 'btn btn-primary px-4 py-3 rounded-lg font-bold text-sm')
          || '    <span class="campaign-card__empty-link">紹介リンク未設定</span>',
        '  </div>',
        '</article>',
  ...
  function renderReferralBenefitsHtml(req) {
    const origin = requestOrigin(req);
    const campaigns = listCampaigns();
    const template = fs.readFileSync(path.join(publicDir, 'referral-benefits.html'), 'utf8');
    const title = `暗号資産取引所 紹介特典比較｜${SITE_NAME}`;
    const description = '国内暗号資産取引所の紹介特典を、招待された方・招待した方の特典、条件、招待コード、公式URL、紹介リンクで横並びに比較できます。';
    const benefitCount = campaigns.filter(campaignHasReferralBenefit).length;
    const affiliateCount = campaigns.filter(campaign => campaign.affiliateUrl).length;
    ...
    return injectCommonDisclosure(template
      .replace(HEAD_META_INJECT, renderHeadMeta({
        title,
        description,
        canonical: '/campaigns/referrals',
        ogImage: '/ogp/default.png',
        pageId: 'campaigns',
        includeDefaultJsonLd: false,
        structuredData: referralBenefitsStructuredData(campaigns, origin, description),
        siteOrigin: origin,
      }))
      .replaceAll('__REFERRAL_CAMPAIGN_COUNT__', String(campaigns.length))
      .replaceAll('__REFERRAL_BENEFIT_COUNT__', String(benefitCount))
      .replaceAll('__REFERRAL_AFFILIATE_COUNT__', String(affiliateCount))
      .replace('<!-- REFERRAL_HIGHLIGHT_CARDS -->', renderReferralHighlightCards(campaigns))
      .replace('<!-- REFERRAL_ROWS -->', renderReferralBenefitRows(campaigns))
      .replace('<!-- REFERRAL_CARDS -->', renderReferralBenefitCards(campaigns)), {
        activeSection: 'campaigns',
        activePath: '/campaigns/referrals',
      });
  }
  ...
  function renderCampaignLinkCards(model) {
    ...
    if (model.affiliateUrl) {
      cards.push({
        ...
        target: model.affiliateTarget === null ? null : (model.affiliateTarget || '_blank'),
        rel: campaignAffiliateRel(model),
        referrerPolicy: model.affiliateReferrerPolicy || null,
        trackingPixelUrl: model.trackingPixelUrl || null,
        cta: '紹介リンクを開く',
      });
    } else {
      cards.push({
  ...
  function renderExchangeConclusionCard(exchange) {
    ...
    return [
      ...
      `    <a class="exchange-hero-cta__text-link" href="${xmlEscape(exchange.officialUrl || '/')}"${exchange.officialUrl ? ' target="_blank" rel="noopener noreferrer"' : ''}>公式サイトで最新条件を確認する</a>`,
      '  </div>',
      ...
      renderExchangeInlineDetails({
        summary: '詳細を見る',
        bodyHtml: `<p>${xmlEscape(costFocusNote)}</p>`,
  ...
  function renderExchangeCostSimulator(exchange) {
    const defaultMarketLabel = exchange.defaultMarket.label || exchange.defaultMarket.instrumentId;
    const quickAmounts = [10000, 100000, 500000, 1000000];
    const officialHref = exchange.officialUrl || '/';
    return [
      `<article class="exchange-cost-simulator" data-exchange-cost-simulator data-exchange-id="${xmlEscape(exchange.id)}" data-instrument-id="${xmlEscape(exchange.defaultMarket.instrumentId)}" data-default-amount="100000">`,
      '  <div class="exchange-cost-simulator__header">',
      '    <div>',
      '      <span class="exchange-cost-simulator__eyebrow">Cost Simulator</span>',
      `      <h3>${xmlEscape(defaultMarketLabel)}をいくら買うかで比較</h3>`,
      '    </div>',
      '    <span class="exchange-cost-simulator__status" data-cost-field="status">初期値を読み込み中</span>',
      '  </div>',
      '  <div class="exchange-cost-simulator__controls">',
      '    <label class="exchange-amount-slider">',
      '      <span>注文金額</span>',
      '      <input type="range" min="10000" max="1000000" step="10000" value="100000" data-cost-amount-range>',
      '    </label>',
      '    <label class="exchange-amount-input">',
      '      <span>JPY</span>',
      '      <input class="field" type="number" min="10000" max="1000000" step="10000" value="100000" inputmode="numeric" data-cost-amount-input>',
      '    </label>',
      ...
      '    </div>',
      '  </div>',
      '  <div class="exchange-cost-bars" aria-hidden="true">',
      ...
      '    <div><span>Sales spread</span><strong data-cost-field="proSalesSpread">-</strong></div>',
      '    <div><span>Board impact</span><strong data-cost-field="proImpact">-</strong></div>',
      '    <div><span>Taker fee</span><strong data-cost-field="proFee">-</strong></div>',
      '  </div>',
      renderExchangeModeComparison(),
      `  <a class="exchange-cost-simulator__official" href="${xmlEscape(officialHref)}"${exchange.officialUrl ? ' target="_blank" rel="noopener noreferrer"' : ''}>公式画面で最終条件を確認</a>`,
      '</article>',
    ].join('\n');
  }
  ...
  function renderExchangeMicroQuote(defaultMarketLabel) {
    return [
      ...
      '    <div><span>Bid</span><strong data-quote-field="bid">-</strong></div>',
      '    <div><span>Ask</span><strong data-quote-field="ask">-</strong></div>',
      '    <div><span>Spread</span><strong data-quote-field="spread">-</strong></div>',
      '  </div>',
      '</div>',
    ].join('\n');
  }
  ...
  function renderExchangeRiskCards(exchange) {
    ...
    return [
      ...
      '  </ol>',
      '</article>',
      '<details class="market-insight-card market-insight-card--summary exchange-risk-disclosure">',
      '  <summary class="exchange-disclosure-card__summary exchange-risk-disclosure__summary">',
      '    <div>',
      '      <p class="market-insight-card__eyebrow">Risk</p>',
      '      <h3 class="market-insight-card__title">注意点・リスク</h3>',
      '      <span class="exchange-risk-disclosure__hint">申込前に開いて確認</span>',
      '    </div>',
      '    <span class="market-insight-card__badge">Check</span>',
      '  </summary>',
    ...
    if (exchange.referralUrl) {
      cards.push({
        ...
        description: exchange.referralCode
          ? `招待コード: ${exchange.referralCode}。紹介特典や適用条件を確認できます。`
