...
function createSiteContentService({
  ...
  function navSectionForPath(value) {
    ...
    if (navPath === '/' || navPath === '/simulator' || navPath === '/sales-spread' || navPath === '/volume-share' || navPath === '/derivatives') {
      return 'comparison';
    }
  ...
  function renderExchangeConclusionCard(exchange) {
    ...
    return UIComponents.renderConclusionCard({
      fragment: true,
      variant: 'exchange',
      eyebrow: 'Exchange Detail',
      title: fallbackText(hero.title, `${exchange.fullName || exchange.label}の特徴まとめ`),
      titleTag: 'h2',
      badgeLabel: 'Summary',
      badgeTone: 'ready',
      lead,
      ...
      facts: [
        ...
        { label: '取引所形式', value: exchangeFormat },
        { label: '基準銘柄', value: exchange.defaultMarket.label || exchange.defaultMarket.instrumentId },
        { label: '取扱銘柄数', value: `${formatCount(exchange.coverage.length)}件` },
        { label: '板取引対応', value: `${formatCount(boardCount)}件` },
        { label: '販売所対応', value: `${formatCount(salesCount)}件` },
      ],
      actions: [
        {
          label: '公式サイトを見る',
  ...
  function renderExchangeOverviewCards(exchange) {
    ...
    const cards = [
      ...
      {
        title: 'データ取得の前提',
        description: `${exchange.dataSourceLabel || '公開API / WebSocket'} をもとに、本サイトで板・出来高・販売所スプレッドの参考データを集計しています。発注前は公式画面の価格と条件を優先してください。`,
        badge: 'Data',
      },
    ];
  ...
  function renderExchangeCoverageCards(exchange) {
    ...
    const coverageOverview = [
      ...
      renderExchangeMarketPills(mainMarkets, {
        emptyMessage: '主要銘柄データを準備中です。',
        showCoverage: true,
      }),
      '</article>',
    ].join('\n');
    ...
    const allCoverage = [
      '<details class="market-definition-card exchange-disclosure-card">',
      '  <summary class="exchange-disclosure-card__summary">',
      '    <div>',
      '      <h3 class="market-definition-card__term">すべての取扱銘柄を見る</h3>',
      '      <span class="market-definition-card__description">全銘柄一覧は必要なときだけ開けます。板・販売所の対応形式もあわせて表示します。</span>',
      '    </div>',
      `    <span class="badge">${xmlEscape(`${formatCount(exchange.coverage.length)}件`)}</span>`,
      '  </summary>',
      renderExchangeMarketPills(exchange.coverage, {
    ...
    const salesCoverage = [
      ...
      '  <summary class="exchange-disclosure-card__summary">',
      '    <div>',
      '      <h3 class="market-definition-card__term">販売所対応銘柄を見る</h3>',
      '      <span class="market-definition-card__description">販売所で使う場合は、銘柄ごとのスプレッドもあわせて見ておきましょう。</span>',
      '    </div>',
      `    <span class="badge">${xmlEscape(`${formatCount(exchange.salesMarkets.length)}件`)}</span>`,
      '  </summary>',
      renderExchangeMarketPills(exchange.salesMarkets, {
    ...
    const groups = [
      {
        ...
        links: [
          ...
          },
          {
            eyebrow: 'Campaign',
            title: '現在のキャンペーン',
            description: `${exchange.campaignLabel}。特典や期間は変わるため、申込前に公式条件を確認してください。`,
            href: exchange.campaignUrl || exchange.officialUrl || '/campaigns',
            cta: '確認する',
          },
        ],
        ...
        links: [
          {
            eyebrow: 'Simulator',
            title: `${exchange.label}で板シミュレーターを開く`,
            description: `${defaultMarketLabel} を、この取引所を選んだ状態で比較できます。`,
            href: queryPath('/simulator', { exchange: exchange.id, market: exchange.defaultMarket.instrumentId }),
            cta: '開く',
          },
          {
            eyebrow: 'Market',
            title: `${defaultMarketLabel}の銘柄ページを見る`,
            description: '代表銘柄を起点に、板・出来高・販売所スプレッドを比較できます。',
            href: marketPath(exchange.defaultMarket.instrumentId),
            cta: '見る',
          },
          {
          ...
          },
          {
            eyebrow: 'Fee',
            title: '手数料比較を見る',
            description: '取引手数料、入出金、暗号資産出金の見方を確認できます。',
            href: '/learn/crypto-fees',
            cta: '読む',
          },
        ],
      },
      {
        ...
        links: [
          ...
          },
          {
            eyebrow: 'Learn',
            title: '取引所の比較方法',
            description: '取扱銘柄、板、スプレッド、手数料、出来高を見る順番を整理しています。',
            href: '/learn/how-to-compare-exchanges',
            cta: '読む',
          },
          {
            eyebrow: 'Learn',
            title: '運営会社・財務情報の見方',
            description: '登録情報、決算公告、親会社、開示資料の見方を確認できます。',
            href: '/learn/exchange-company-analysis',
            cta: '読む',
          },
        ].concat(aboutArticle ? [{
          eyebrow: 'Policy',
  ...
  function renderExchangeHtml(req, exchange) {
    ...
    const title = `${pageExchange.label} 特徴・手数料・取扱銘柄・信頼性｜${SITE_NAME}`;
    const description = `${pageExchange.label} の特徴、向いている人、手数料・スプレッド、板取引、取扱銘柄、入出金条件、運営会社・信頼性を確認できる取引所詳細ページです。`;
