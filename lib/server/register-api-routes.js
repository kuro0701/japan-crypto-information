function registerApiRoutes(app, {
  ...
  app.get('/api/volume-share/monthly', (req, res) => {
    res.json(volumeShareStore.getMonthlyShare({
      ...spotVolumeOptions,
      months: req.query.months || 12,
    }));
  });
  ...
  app.get('/api/article-market-reference/:ticker', async (req, res) => {
    const ticker = String(req.params.ticker || '').trim().toUpperCase();
    if (!marketReferenceClient || !marketReferenceClient.supports(ticker)) {
      res.status(404).json({ error: 'Market reference not supported' });
      return;
    }

    try {
      const reference = await marketReferenceClient.getMarketReference(ticker);
      res.set('Cache-Control', 'public, max-age=5, stale-while-revalidate=15');
      res.json(reference);
    } catch (error) {
      console.warn(`[Market Reference] ${ticker}:`, error.message);
      res.status(502).json({ error: 'Market reference temporarily unavailable' });
    }
  });

  app.get('/api/article-onchain/tron', async (_req, res) => {
    if (!tronOnchainClient) {
      res.status(404).json({ error: 'TRON onchain reference not supported' });
      return;
    }

    try {
      const snapshot = await tronOnchainClient.getSnapshot();
      res.set('Cache-Control', 'public, max-age=120, stale-while-revalidate=300');
      res.json(snapshot);
    } catch (error) {
      console.warn('[TRON Onchain]:', error.message);
      res.status(502).json({ error: 'TRON onchain reference temporarily unavailable' });
    }
  });
