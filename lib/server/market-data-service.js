...
function createMarketDataService({
  ...
  function exchangeDetailPath(exchangeId) {
    const slug = exchangePageSlug(exchangeId);
    return `/exchanges/${encodeURIComponent(slug || exchangeId)}`;
  }

  function exchangeActionFields(exchangeId) {
    const content = getExchangePageContent(exchangeId) || {};
    const referralUrl = content.referralUrl || null;
    return {
      detailPath: exchangeDetailPath(exchangeId),
      officialUrl: content.officialUrl || null,
      signupUrl: referralUrl || content.signupUrl || content.officialUrl || null,
      referralUrl,
      referralRel: content.referralRel || null,
      referralReferrerPolicy: content.referralReferrerPolicy || null,
      referralTarget: Object.prototype.hasOwnProperty.call(content, 'referralTarget') ? content.referralTarget : undefined,
      referralTrackingPixelUrl: content.referralTrackingPixelUrl || null,
    };
  }
  ...
  function pickPreferredRow(rows, valueSelector, direction = 'min') {
    const candidates = (rows || [])
      .map((row) => ({
        row,
        value: parseNullableNumber(valueSelector(row)),
      }))
  ...
  function pickSalesSpreadPct(row) {
    if (!row) return null;
    const latestPct = parseNullableNumber(row.latest && row.latest.spreadPct);
    if (latestPct != null) return latestPct;
    const oneDayPct = parseNullableNumber(row.averages && row.averages['1d'] && row.averages['1d'].spreadPct);
    if (oneDayPct != null) return oneDayPct;
    return parseNullableNumber(row.averages && row.averages['7d'] && row.averages['7d'].spreadPct);
  }
  ...
  function buildDomesticSalesRow(row) {
    ...
    const hasSpreadPct = parseNullableNumber(spreadPct) != null;
    return {
      status: hasSpreadPct ? 'ready' : 'waiting',
      message: hasSpreadPct ? null : '販売所価格を取得中',
  ...
  function buildDomesticCostRow(row) {
    ...
    return {
      ...
      feesJPY: result ? result.feesJPY : null,
      feeRatePct: result ? result.feeRatePct : null,
      slippageFromBestJPY: result ? result.slippageFromBestJPY : null,
      slippageFromBestPct: result ? result.slippageFromBestPct : null,
  ...
  function buildDomesticOrderbookRow(row) {
    ...
    return {
      ...
      totalAskDepthJPY: row.totalAskDepthJPY ?? null,
      totalBidDepthJPY: row.totalBidDepthJPY ?? null,
      askDepthLevels: Array.isArray(row.askDepthLevels) ? row.askDepthLevels : [],
      bidDepthLevels: Array.isArray(row.bidDepthLevels) ? row.bidDepthLevels : [],
  ...
  function buildMarketDomesticComparison(market, options = {}) {
    ...
    const rows = sortDomesticComparisonRows((market.exchanges || []).map((exchange) => {
      ...
      return {
        ...
        actions: exchangeActionFields(exchange.id),
      };
    }));
  ...
  function resultSummary(result) {
    ...
    return {
      ...
      slippageFromBestJPY: result.slippageFromBestJPY,
