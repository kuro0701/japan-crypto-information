...
const { generateSalesSpreadInsights } = require('./sales-spread-insights');
...
function getSnapshotDateJst(snapshot) {
  ...
  if (snapshot.spreadDateJst) return snapshot.spreadDateJst;
...
class SalesSpreadStore {
  ...
  queuePersistence() {
    ...
    this.persistenceWriteChain = this.persistenceWriteChain
      ...
      .catch((err) => {
        console.warn('[Sales Spread] Could not persist history to snapshot store:', err.message);
      });
  ...
  captureDaily(records, meta = {}) {
    ...
    const existingSnapshot = this.getDailySnapshot(spreadDateJst);
  ...
  getHistoricalSnapshots(windowDays, options = {}) {
    const { stableSnapshots } = this.getSnapshotAvailability(options);
  ...
  getWindowRecords(windowKey, options = {}) {
    ...
    const snapshots = this.getHistoricalSnapshots(windowDays, options);
  ...
  getReport(options = {}) {
    ...
    for (const windowKey of REPORT_WINDOWS) {
      const windowData = this.getWindowRecords(windowKey, options);
      windows[windowKey] = {
        source: windowData.source,
        sampleSnapshotCount: windowData.snapshots.length,
        earliestSpreadDateJst: windowData.snapshots[0] ? getSnapshotDateJst(windowData.snapshots[0]) : null,
        latestSpreadDateJst: windowData.snapshots[windowData.snapshots.length - 1]
          ? getSnapshotDateJst(windowData.snapshots[windowData.snapshots.length - 1])
          : null,
      };
    ...
    return {
      meta: {
        ...
        latestRecordedSpreadDateJst: snapshotAvailability.latestRecordedDateJst,
        latestProvisionalSpreadDateJst: snapshotAvailability.latestProvisionalDateJst,
        latestPartialSpreadDateJst: snapshotAvailability.latestPartialDateJst,
        recordCount: rows.length,
        refreshStatus: this.refreshStatus,
        windows,
      },
      quality: this.buildQualityRows(this.getWindowRecords('1d', options).records),
      rows,
    };
  }

  buildHistoryRows(snapshots) {
    const rows = [];
  ...
  getHistory(windowKey = '30d', options = {}) {
    ...
    const rows = this.buildHistoryRows(snapshots);
    ...
    return {
      meta: {
        ...
        earliestSpreadDateJst: snapshots[0] ? getSnapshotDateJst(snapshots[0]) : null,
  ...
  getInsights(windowKey = '30d', options = {}) {
    const { filters = {}, insightConfig = {}, ...historyOptions } = options || {};
    const normalizedWindow = WINDOW_DAYS[windowKey] ? windowKey : '30d';
    const windowDays = WINDOW_DAYS[normalizedWindow];
    const periods = normalizePositiveInteger(insightConfig.periods, 1, 1, HISTORY_RETENTION_DAYS - 1);
    const zscoreWindow = normalizePositiveInteger(insightConfig.window, 8, 2, HISTORY_RETENTION_DAYS - 1);
    const requestedSnapshotDays = Math.min(
      HISTORY_RETENTION_DAYS,
      Math.max(windowDays, periods + 1, zscoreWindow + 1)
    );
    const generatedAt = historyOptions.now ? new Date(historyOptions.now).toISOString() : new Date().toISOString();
    const snapshotAvailability = this.getSnapshotAvailability({ now: generatedAt });
    let snapshots = this.getHistoricalSnapshots(requestedSnapshotDays, historyOptions);
    let source = 'daily-snapshots';
    ...
    return {
      meta: {
        ...
        recordCount: rows.length,
        ...result.meta,
        insightWindowKey: normalizedWindow,
        insightWindowDays: windowDays,
        requestedSnapshotDays,
      },
