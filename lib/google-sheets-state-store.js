...
function monthlySnapshotEntryKey(snapshot, index) {
  return String((snapshot && snapshot.monthJst) || `month-${index}`);
}
...
function encodeStateRows(stateKey, payload, updatedAt) {
  ...
  const normalized = {
    ...
    monthlySnapshots: Array.isArray(payload && payload.monthlySnapshots) ? payload.monthlySnapshots : [],
  };
  ...
  normalized.monthlySnapshots.forEach((snapshot, index) => {
    appendEntry('monthly', monthlySnapshotEntryKey(snapshot, index), snapshot);
  });
...
class GoogleSheetsStateStore {
  ...
  decodeRows(stateKey, rows) {
    ...
    for (const entry of entries.values()) {
      ...
      } else if (entry.entryType === 'monthly') {
        monthlyEntries.push({
          key: entry.entryKey,
          payload: parsed,
        });
    ...
    const decoded = {
      version: monthlyEntries.length > 0 ? 2 : 1,
      latest,
    ...
    if (monthlyEntries.length > 0) {
      decoded.monthlySnapshots = monthlyEntries
        .sort((a, b) => String(a.key).localeCompare(String(b.key)))
        .map(entry => entry.payload);
    }
    return decoded;
