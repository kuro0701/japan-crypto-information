...
function spreadMovementInsights(latest, config, terms) {
  ...
  if (narrower.spreadChange < 0) {
    insights.push(createSpreadMovementInsight({
      ...
      priorityBase: 90,
      terms,
      messageJa: `${terms.currentLabel}の最大のコスト改善は ${subjectLabel(narrower)}（${costChangePhrase(narrower.spreadChange, terms)}、${fmtPct(previousSpread(narrower))} → ${fmtPct(narrower.spreadPct)}）です。`,
  ...
  if (wider.spreadChange > 0 && !seenEntityIds.has(wider.entityId)) {
    insights.push(createSpreadMovementInsight({
      ...
      priorityBase: 88,
      terms,
      messageJa: `${terms.currentLabel}の最大のコスト悪化は ${subjectLabel(wider)}（${costChangePhrase(wider.spreadChange, terms)}、${fmtPct(previousSpread(wider))} → ${fmtPct(wider.spreadPct)}）です。`,
  ...
  rows
    ...
    .forEach((row) => {
      ...
      insights.push(createSpreadMovementInsight({
        ...
        terms,
        messageJa: `${subjectLabel(row)} のコストは${costChangePhrase(row.spreadChange, terms)}（${fmtPct(previousSpread(row))} → ${fmtPct(row.spreadPct)}）です。`,
...
function narrowestInsights(latest, previous, config) {
  ...
  if (isFiniteNumber(gapChange) && Math.abs(gapChange) >= config.gapChangeThreshold) {
    ...
    return [createInsight({
      ...
      messageJa: `${subjectLabel(latestLeader)} は最狭を維持していますが、2番手との差は前回の ${fmtSpreadGap(previousSecondGap)} から ${fmtSpreadGap(latestSecondGap)} へ${directionJa}しています。`,
      metadata: {
...
function rankMovementInsights(latest, config, terms) {
  ...
  return material.slice(0, config.maxRankMovementInsights).map((row) => {
    ...
    return createInsight({
      ...
      messageJa: `${subjectLabel(row)} は${row.rankPrev}位から${row.rank}位へ${directionJa}しました（${costChangePhrase(row.spreadChange, terms)}）。`,
...
function gapChangeInsights(latest, config) {
  ...
  if (leaderGapRows.length > 0) {
    ...
    insights.push(createInsight({
      ...
      messageJa: `${subjectLabel(row)} は最狭との差が前回の ${fmtSpreadGap(row.gapToNarrowestPrev)} から ${fmtSpreadGap(row.gapToNarrowest)} へ${directionJa}しています。`,
  ...
  if (aboveGapRows.length > 0) {
    ...
    insights.push(createInsight({
      ...
      messageJa: `${subjectLabel(row)} は直上順位との差が前回の ${fmtSpreadGap(row.gapToAbovePrev)} から ${fmtSpreadGap(row.gapToAbove)} へ${directionJa}しています。`,
...
function zscoreInsights(latest, config, terms) {
  return latest
    ...
    .map((row) => {
      ...
      const stateJa = direction === 'widen'
        ? 'コストが高騰しています（異常値）'
        : 'コストが大きく下がっています（通常より安い状態）';
      return createInsight({
        type: 'zscore_outlier',
        row,
        metric: 'zscore',
        ...
        messageJa: `${subjectLabel(row)} のスプレッドは${rollingWindowLabel(config, terms)}の平均から大きく外れて、${stateJa}。`,
...
function comparisonPrefix(terms) {
  const label = terms && terms.comparisonLabel;
  if (label === '前日比') return '前日より';
  if (label === '7日前比') return '7日前より';
  if (label === '30日前比') return '30日前より';
  return '前回より';
}

function fmtApproxPct(value) {
...
function costChangePhrase(value, terms) {
  if (!isFiniteNumber(value)) return '変化を確認中';
  return `${comparisonPrefix(terms)}${fmtApproxPct(value)}${value > 0 ? '悪化' : '改善'}`;
}

function fmtSpreadGap(value) {
  if (!isFiniteNumber(value)) return '-';
  return `${fmtApproxPct(value)}分`;
}

function rollingWindowLabel(config, terms) {
  if (terms && terms.streakUnit === '日') return `過去${config.window}日間`;
  return `過去${config.window}${terms && terms.streakUnit ? terms.streakUnit : '期間'}`;
}
