...
class VolumeShareStore {
  ...
  getSnapshotAvailability(options = {}) {
    ...
    return {
      ...
      totalSnapshotCount: this.data.dailySnapshots.length,
      latestRecordedDateJst: this.data.dailySnapshots.length > 0
        ? getSnapshotDateJst(this.data.dailySnapshots[this.data.dailySnapshots.length - 1])
        : null,
  ...
  getShare(windowKey = '1d', options = {}) {
    ...
    if (filteredRecords.length === 0 && typeof recordFilter === 'function' && source !== 'latest') {
      const latestRecords = this.getLatestRecords().filter(record => recordMatchesFilter(record, recordFilter));
  ...
  getHistory(windowKey = '30d', options = {}) {
    ...
    if (rows.length === 0 && typeof recordFilter === 'function' && this.data.latest && Array.isArray(this.data.latest.records)) {
