class OrderBook {
  ...
  static sampleDepthLevels(levels, sampleSize) {
    return levels.slice(0, sampleSize).map(level => ({
      price: level.price,
      quantity: level.quantity,
      depthJPY: level.price * level.quantity,
      orders: level.orders,
    }));
  }
  ...
  toSummary(options = {}) {
    ...
    const depthSampleSize = Number.isFinite(Number(options.depthSampleSize))
      ? Math.max(0, Math.min(16, Math.floor(Number(options.depthSampleSize))))
      : 8;
    return {
      ...
      askDepthLevels: OrderBook.sampleDepthLevels(this.asks, depthSampleSize),
      bidDepthLevels: OrderBook.sampleDepthLevels(this.bids, depthSampleSize),
