...
function symbolToInstrumentId(symbol) {
  ...
  if (!text) return '';
  const derivativeMatch = text.match(/^([A-Z0-9]+)_([A-Z0-9]+)$/);
  if (derivativeMatch) return `${derivativeMatch[1]}-CFD-${derivativeMatch[2]}`;
  return `${text}-JPY`;
}

function instrumentIdToSymbol(instrumentId) {
  const text = String(instrumentId || '').toUpperCase();
  const derivativeMatch = text.match(/^([A-Z0-9]+)-CFD-([A-Z0-9]+)$/);
  if (derivativeMatch) return `${derivativeMatch[1]}_${derivativeMatch[2]}`;
  return text.split('-')[0] || '';
}
...
function symbolToMarket(symbolOrRaw) {
  ...
  const derivativeMatch = symbol.match(/^([A-Z0-9]+)_([A-Z0-9]+)$/);
  const isDerivative = Boolean(derivativeMatch);
  const baseCurrency = isDerivative ? derivativeMatch[1] : symbol;
  const quoteCurrency = isDerivative ? derivativeMatch[2] : 'JPY';
  if (quoteCurrency !== 'JPY') return null;

  const instrumentId = symbolToInstrumentId(symbol);
  return {
    instrumentId,
    label: isDerivative ? `${baseCurrency}-CFD/${quoteCurrency}` : `${baseCurrency}/${quoteCurrency}`,
    baseCurrency,
    quoteCurrency,
    marketType: isDerivative ? 'derivative' : 'spot',
    marketTypeLabel: isDerivative ? '暗号資産FX' : '現物',
    derivativeType: isDerivative ? 'cfd' : null,
    underlyingInstrumentId: isDerivative ? `${baseCurrency}-${quoteCurrency}` : null,
    minSize: raw.minOrderSize || null,
    sizeIncrement: raw.sizeStep || null,
    tickSize: raw.tickSize || null,
    takerFeeRate: Number.isFinite(Number(raw.takerFee)) ? Number(raw.takerFee) : null,
    takerFeeNote: isDerivative ? '暗号資産FX 通常' : '現物 JPY 通常',
    status: 'active',
  };
}
...
class GMOClient extends EventEmitter {
  ...
  async fetchTicker(instrumentId) {
    try {
      ...
      const ticker = this.normalizeTicker(instrumentId, body.data[0]);
