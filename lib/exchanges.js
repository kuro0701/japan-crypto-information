function buildStaticMarket(instrumentId, overrides = {}) {
  const parts = String(instrumentId || '').split('-').filter(Boolean);
  const baseCurrency = overrides.baseCurrency || parts[0] || '';
  const quoteCurrency = overrides.quoteCurrency || parts[parts.length - 1] || '';
  const marketType = overrides.marketType || 'spot';
  return {
    instrumentId,
    label: overrides.label || (baseCurrency && quoteCurrency ? `${baseCurrency}/${quoteCurrency}` : instrumentId),
    baseCurrency,
    quoteCurrency,
    marketType,
    marketTypeLabel: overrides.marketTypeLabel || (marketType === 'derivative' ? 'デリバティブ' : '現物'),
...
const BITFLYER_CRYPTO_CFD_TAKER_FEE_RATE = 0;
const BITBANK_TAKER_FEE_RATE = 0.0012;
const GMO_TAKER_FEE_RATE = 0.0003;
const GMO_DERIVATIVE_TAKER_FEE_RATE = 0;
const BINANCE_JAPAN_TAKER_FEE_RATE = 0.001;
const BITTRADE_TAKER_FEE_RATE = 0.001;
const DEFAULT_FUNDING_SUPPORT = Object.freeze({
  fiatDeposit: true,
  fiatWithdrawal: true,
  cryptoDeposit: true,
  cryptoWithdrawal: true,
  summary: 'JPY入出金 / 暗号資産入出庫',
  statusLabel: '入出金対応',
  note: '銀行・銘柄・ネットワーク別の手数料と停止状況は公式確認',
});
...
const BITFLYER_MARKET_OVERRIDES = {
  ...
  'BTC-CFD-JPY': {
    ...
    tickSize: 1,
    takerFeeRate: BITFLYER_CRYPTO_CFD_TAKER_FEE_RATE,
    takerFeeNote: 'Crypto CFD 取引手数料無料',
  },
};
...
const DEFAULT_GMO_SPOT_MARKETS = buildStaticMarkets([
  'BTC-JPY',
  'ETH-JPY',
  'BCH-JPY',
  'LTC-JPY',
...
const DEFAULT_GMO_DERIVATIVE_BASES = [
  'BTC',
  'ETH',
  'BCH',
  'LTC',
...
const DEFAULT_GMO_DERIVATIVE_MARKETS = buildStaticMarkets(
  DEFAULT_GMO_DERIVATIVE_BASES.map(baseCurrency => `${baseCurrency}-CFD-JPY`),
  DEFAULT_GMO_DERIVATIVE_BASES.reduce((acc, baseCurrency) => {
    acc[`${baseCurrency}-CFD-JPY`] = {
      label: `${baseCurrency}-CFD/JPY`,
      baseCurrency,
      quoteCurrency: 'JPY',
      marketType: 'derivative',
      marketTypeLabel: '暗号資産FX',
      derivativeType: 'cfd',
      underlyingInstrumentId: `${baseCurrency}-JPY`,
      takerFeeRate: GMO_DERIVATIVE_TAKER_FEE_RATE,
      takerFeeNote: '暗号資産FX 通常',
    };
    return acc;
  }, {})
);
const DEFAULT_GMO_MARKETS = DEFAULT_GMO_SPOT_MARKETS.concat(DEFAULT_GMO_DERIVATIVE_MARKETS);
...
function normalizeMarket(instrument) {
  ...
  const parts = String(instrumentId || '').split('-').filter(Boolean);
  const baseCurrency = instrument.base_currency || instrument.baseCurrency || parts[0] || '';
  const quoteCurrency = instrument.quote_currency || instrument.quoteCurrency || parts[parts.length - 1] || '';
  const marketType = instrument.market_type || instrument.marketType || 'spot';
  ...
  return {
    ...
    quoteCurrency,
    marketType,
    marketTypeLabel: instrument.market_type_label || instrument.marketTypeLabel || (marketType === 'derivative' ? 'デリバティブ' : '現物'),
    derivativeType: instrument.derivative_type || instrument.derivativeType || null,
    underlyingInstrumentId: instrument.underlying_instrument_id || instrument.underlyingInstrumentId || null,
