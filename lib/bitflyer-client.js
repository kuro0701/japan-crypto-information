...
function productCodeToInstrumentId(productCode) {
  const text = String(productCode || '').toUpperCase();
  const fxMatch = text.match(/^FX_([A-Z0-9]+)_([A-Z0-9]+)$/);
  if (fxMatch) return `${fxMatch[1]}-CFD-${fxMatch[2]}`;
  return text.replace(/_/g, '-');
}

function instrumentIdToProductCode(instrumentId) {
  const text = String(instrumentId || '').toUpperCase();
  const cfdMatch = text.match(/^([A-Z0-9]+)-CFD-([A-Z0-9]+)$/);
  if (cfdMatch) return `FX_${cfdMatch[1]}_${cfdMatch[2]}`;
...
function instrumentIdToMarket(instrumentId, raw = {}) {
  ...
  if (cfdMatch) {
    const [, baseCurrency, quoteCurrency] = cfdMatch;
    return {
      instrumentId,
      label: `${baseCurrency}-CFD/${quoteCurrency}`,
      baseCurrency,
      quoteCurrency,
      marketType: 'derivative',
      marketTypeLabel: 'Crypto CFD',
      derivativeType: 'cfd',
      underlyingInstrumentId: `${baseCurrency}-${quoteCurrency}`,
      minSize: 0.001,
      sizeIncrement: 0.00000001,
      tickSize: 1,
      takerFeeRate: 0,
      takerFeeNote: 'Crypto CFD 取引手数料無料',
      status: raw.market_type === 'FX' || !raw.market_type ? 'active' : String(raw.market_type).toLowerCase(),
    };
  }

  const parts = String(instrumentId || '').split('-');
  const baseCurrency = parts[0] || '';
  const quoteCurrency = parts[parts.length - 1] || '';
  return {
    instrumentId,
...
function productToMarket(raw = {}) {
  ...
  if (!productCode || !instrumentId.endsWith('-JPY')) return null;
  if (raw.market_type !== 'Spot' && raw.market_type !== 'FX') return null;
...
class BitflyerClient extends EventEmitter {
  ...
  async fetchTicker(instrumentId) {
    try {
      ...
      const data = await res.json();
      const ticker = this.normalizeTicker(data);
