...
function topMoverInsights(latest, config, terms) {
  ...
  if (gainer.shareChange > 0) {
    insights.push(createShareMovementInsight({
      ...
      priorityBase: 90,
      terms,
      messageJa: `${gainer.exchangeLabel} のシェアは${terms.comparisonLabel} ${fmtPt(gainer.shareChange)}（${fmtShareTransition(gainer, 'up')}）しています。`,
  ...
  if (loser.shareChange < 0 && !seenExchangeIds.has(loser.exchangeId)) {
    insights.push(createShareMovementInsight({
      ...
      priorityBase: 88,
      terms,
      messageJa: `${loser.exchangeLabel} のシェアは${terms.comparisonLabel} ${fmtPt(loser.shareChange)}（${fmtShareTransition(loser, 'down')}）しています。`,
