...
function topMoverInsights(latest, config, terms) {
  ...
  if (gainer.shareChange > 0) {
    insights.push(createShareMovementInsight({
      ...
      priorityBase: 90,
      terms,
      messageJa: shareMovementMessage(gainer, 'up', terms),
  ...
  if (loser.shareChange < 0 && !seenExchangeIds.has(loser.exchangeId)) {
    insights.push(createShareMovementInsight({
      ...
      priorityBase: 88,
      terms,
      messageJa: shareMovementMessage(loser, 'down', terms),
  ...
  rows
    ...
    .forEach((row) => {
      ...
      insights.push(createShareMovementInsight({
        ...
        terms,
        messageJa: shareMovementMessage(row, direction, terms),
  ...
  for (const row of latest) {
    if (row.increaseStreak >= config.minStreak) {
      insights.push(createInsight({
        ...
        messageJa: `${row.exchangeLabel} はシェアが${row.increaseStreak}${terms.streakUnit}連続で上昇。`,
    ...
    if (row.decreaseStreak >= config.minStreak) {
      insights.push(createInsight({
        ...
        messageJa: `${row.exchangeLabel} はシェアが${row.decreaseStreak}${terms.streakUnit}連続で低下。`,
        metadata: { dedupeKey: `streak:${row.exchangeId}` },
      }));
    }
  }
...
function shareMovementMessage(row, direction, terms) {
  const particle = direction === 'up' ? 'が' : 'は';
  return `${row.exchangeLabel} のシェア${particle}${terms.comparisonLabel} ${fmtPt(row.shareChange)}（${fmtShareTransition(row, direction)}）。`;
}

function fmtShareTransition(row, direction) {
  const directionJa = direction === 'down' ? '低下' : '拡大';
  return `${fmtShare(previousShare(row))} から ${fmtShare(row.volumeShare)} へ${directionJa}`;
}
