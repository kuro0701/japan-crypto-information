...
function normalizeConfig(config = {}) {
  return {
    ...
    maxShareMovementInsights: normalizePositiveInteger(
      config.maxShareMovementInsights,
      DEFAULT_CONFIG.maxShareMovementInsights,
      1,
      12
    ),
...
function buildExchangeShareFrame(historyRows = [], filters = {}) {
  ...
  for (const date of dates) {
    const count = allRows.filter(row => row.date === date).length;
    byDate.get(date).exchangeCount = count;
  }

  const rows = exchangeFilter
    ? allRows.filter(row => row.exchangeId === exchangeFilter)
    : allRows;
...
function generateVolumeShareInsights(historyRows = [], options = {}) {
  ...
  const candidates = [
    ...
    ...(config.includeRankInsights ? leaderInsights(latest, previous, config) : []),
    ...
    ...(frame.filters.exchangeId ? [] : marketStructureInsights(concentration, latestDate, config, terms)),
  ];
...
function topMoverInsights(latest, config, terms) {
  const rows = latest.filter(row => (
    isFiniteNumber(row.shareChange)
    && isFiniteNumber(row.volumeShare)
    && Math.abs(row.shareChange) >= config.minShareChange
  ));
  ...
  if (gainer.shareChange > 0) {
    insights.push(createShareMovementInsight({
      row: gainer,
      type: 'top_gainer',
      direction: 'up',
      priorityBase: 90,
      terms,
      messageJa: `${terms.currentLabel}の最大シェア増加は ${gainer.exchangeLabel}（${terms.comparisonLabel} ${fmtPt(gainer.shareChange)}、${fmtShareTransition(gainer, 'up')}）です。`,
      dedupeKey: `share_change:${gainer.exchangeId}`,
    }));
    seenExchangeIds.add(gainer.exchangeId);
  }

  if (loser.shareChange < 0 && !seenExchangeIds.has(loser.exchangeId)) {
    insights.push(createShareMovementInsight({
      row: loser,
      type: 'top_loser',
      direction: 'down',
      priorityBase: 88,
      terms,
      messageJa: `${terms.currentLabel}の最大シェア低下は ${loser.exchangeLabel}（${terms.comparisonLabel} ${fmtPt(loser.shareChange)}、${fmtShareTransition(loser, 'down')}）です。`,
      dedupeKey: `share_change:${loser.exchangeId}`,
    }));
    seenExchangeIds.add(loser.exchangeId);
  }
  ...
  rows
    ...
    .forEach((row) => {
      if (insights.length >= config.maxShareMovementInsights) return;
      if (seenExchangeIds.has(row.exchangeId)) return;
      const direction = row.shareChange > 0 ? 'up' : 'down';
      insights.push(createShareMovementInsight({
        row,
        type: direction === 'up' ? 'share_up' : 'share_down',
        direction,
        priorityBase: direction === 'up' ? 82 : 80,
        terms,
        messageJa: `${row.exchangeLabel} のシェアは${terms.comparisonLabel} ${fmtPt(row.shareChange)}（${fmtShareTransition(row, direction)}）しています。`,
        dedupeKey: `share_change:${row.exchangeId}`,
      }));
      seenExchangeIds.add(row.exchangeId);
    });

  return insights;
}

function createShareMovementInsight({
  row,
  type,
  direction,
  priorityBase,
  ...
  return createInsight({
    ...
    metric: 'share_change',
    value: row.shareChange,
    unit: 'pt',
    direction,
    priority: priorityBase + Math.min(Math.floor(Math.abs(row.shareChange) * 1000), 9),
    messageJa,
    metadata: {
      previousShare: previousShare(row),
      latestShare: row.volumeShare,
  ...
  for (const row of latest) {
    if (row.increaseStreak >= config.minStreak) {
      insights.push(createInsight({
        ...
        messageJa: `${row.exchangeLabel} はシェアが${row.increaseStreak}${terms.streakUnit}連続で上昇しています。`,
    ...
    if (row.decreaseStreak >= config.minStreak) {
      insights.push(createInsight({
        ...
        messageJa: `${row.exchangeLabel} はシェアが${row.decreaseStreak}${terms.streakUnit}連続で低下しています。`,
        metadata: { dedupeKey: `streak:${row.exchangeId}` },
      }));
    }
  }
...
function zscoreInsights(latest, config, terms) {
  return latest
    ...
    .map((row) => {
      ...
      const movementJa = direction === 'up' ? '急上昇中' : '急低下中';
      const comparisonJa = direction === 'up' ? '大きく上回る' : '大きく下回る';
      return createInsight({
        type: 'zscore_outlier',
        exchange: row.exchangeLabel,
        exchangeId: row.exchangeId,
        ...
        messageJa: `${row.exchangeLabel} の出来高シェアが${movementJa}（過去${config.window}${terms.streakUnit}間の平均を${comparisonJa}急激な動きです）。`,
...
function marketStructureInsights(concentration, latestDate, config, terms) {
  ...
  const candidates = [];
  [
    ['top3Share', 'top3ShareChange', '上位3社への集中度'],
    ['top5Share', 'top5ShareChange', '上位5社への集中度'],
    ...
    const directionJa = direction === 'concentrating'
      ? '取引が上位の取引所に集まる傾向が見られます'
      : '取引がさらに他の取引所へ分散する傾向が見られます';
    candidates.push(createInsight({
      ...
      messageJa: `${label}が${terms.comparisonLabel} ${fmtPt(change)} となり、${directionJa}。`,
  ...
  if (isFiniteNumber(latest.hhiChange) && Math.abs(latest.hhiChange) >= config.hhiChangeThreshold) {
    ...
    candidates.push(createInsight({
      ...
      messageJa: `HHIは${terms.comparisonLabel} ${fmtDecimal(latest.hhiChange)} となり、${directionJa}。`,
      metadata: { dedupeKey: 'market_structure' },
    }));
  }
...
function previousShare(row) {
  if (!row || !isFiniteNumber(row.volumeShare) || !isFiniteNumber(row.shareChange)) return null;
  return row.volumeShare - row.shareChange;
}
...
function fmtShareTransition(row, direction) {
  const directionJa = direction === 'down' ? '縮小' : '拡大';
  return `${fmtShare(previousShare(row))} から ${fmtShare(row.volumeShare)} に${directionJa}`;
}
