...
function topMoverInsights(latest, config, terms) {
  ...
  if (gainer.shareChange > 0) {
    insights.push(createShareMovementInsight({
      ...
      priorityBase: 90,
      terms,
      messageJa: `${terms.currentLabel}の最大シェア増加は ${gainer.exchangeLabel}（${terms.comparisonLabel} ${fmtPt(gainer.shareChange)}、${fmtShare(previousShare(gainer))} → ${fmtShare(gainer.volumeShare)}）です。`,
  ...
  if (loser.shareChange < 0 && !seenExchangeIds.has(loser.exchangeId)) {
    insights.push(createShareMovementInsight({
      ...
      priorityBase: 88,
      terms,
      messageJa: `${terms.currentLabel}の最大シェア低下は ${loser.exchangeLabel}（${terms.comparisonLabel} ${fmtPt(loser.shareChange)}、${fmtShare(previousShare(loser))} → ${fmtShare(loser.volumeShare)}）です。`,
  ...
  rows
    ...
    .forEach((row) => {
      ...
      insights.push(createShareMovementInsight({
        ...
        terms,
        messageJa: `${row.exchangeLabel} のシェアは${terms.comparisonLabel} ${fmtPt(row.shareChange)}（${fmtShare(previousShare(row))} → ${fmtShare(row.volumeShare)}）で${directionJa}しています。`,
