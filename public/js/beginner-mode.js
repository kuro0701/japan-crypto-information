(() => {
  ...
  const TERMS = {
    ...
    bid: {
      title: 'Bid',
      description: '買いたい人が出している価格です。売る時は、基本的に高いBidがあるほど有利です。',
    },
    ask: {
      title: 'Ask',
      description: '売りたい人が出している価格です。買う時は、基本的に低いAskがあるほど有利です。',
    },
    'best-bid-ask': {
      title: '最良Bid / Ask',
    ...
    cfd: {
      title: 'CFD / FX',
      description: '暗号資産そのものを受け渡しせず、価格差を対象に売買する取引です。レバレッジ、証拠金、ロスカット条件の確認が重要です。',
    },
    slippage: {
      title: 'スリッページ',
    ...
    taker: {
      title: 'taker / 成行注文側',
      description: '板にすでに並んでいる注文を成行などで消費して、すぐ買う・すぐ売る側です。急いで約定する代わりに、板が薄いと価格が動きやすくなります。',
    },
    'sales-spread': {
      title: '販売所スプレッド',
    ...
    vwap: {
      title: 'VWAP / 実質平均価格',
      description: '約定した価格を数量で平均した価格です。このページの比較では手数料込みの実質購入価格を見る目安として使います。',
    },
    impact: {
      title: 'Impact',
    ...
    'seed-phrase': {
      title: 'シードフレーズ',
      description: 'ウォレットを復元するための複数単語の控えです。知られると資産を動かされる可能性があるため、誰にも渡してはいけません。',
    },
    'implied-volatility': {
      title: 'インプライド・ボラティリティ',
      description: 'オプション市場などから逆算される、将来の値動きの大きさの見込みです。過去の値動きそのものではありません。',
    },
    'market-cap': {
      title: '時価総額',
      description: '価格に流通数量を掛けた市場規模の目安です。参照する価格や供給量によって差が出ることがあります。',
    },
    liquidity: {
      title: '流動性',
  ...
  function guideProfile() {
    ...
    if (path === '/markets' || path === '/markets.html') {
      return {
        ...
        summary: '最初は「対応取引所数」「24h変化」「スプレッド警告」を見て、買いたい銘柄を選びます。ONの間は広い販売所スプレッドをカード上で強調します。',
        metrics: ['対応取引所数', '24h変化', 'スプレッド警告'],
