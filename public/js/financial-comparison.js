(function () {
  ...
  function renderBalanceShapeCard(company, maxTotalAssets) {
    ...
    return [
      ...
      '  </div>',
      '  <div class="financial-shape-visual">',
      totalAssets == null ? '' : [
        '    <div class="financial-shape-total-badge">',
        '      <span>総資産</span>',
        `      <strong>${escapeHtml(compactMetricValue(totalAssets, 'totalAssets'))}</strong>`,
        '    </div>',
      ].join('\n'),
      ...
      totalAssets == null ? '      <span class="financial-shape-box__empty">総資産未開示</span>' : [
        '      <span class="financial-shape-segment financial-shape-segment--liability" aria-hidden="true"></span>',
        '      <span class="financial-shape-segment financial-shape-segment--equity" aria-hidden="true"></span>',
      ].join('\n'),
      '    </div>',
      '  </div>',
      '  <dl class="financial-shape-card__facts">',
      `    <div class="financial-shape-fact financial-shape-fact--liability"><dt>負債</dt><dd>${escapeHtml(formatMetricValue(liabilities, 'totalAssets'))}</dd></div>`,
      `    <div class="financial-shape-fact financial-shape-fact--equity"><dt>純資産</dt><dd>${escapeHtml(formatMetricValue(netAssets, 'netAssets'))}</dd></div>`,
      `    <div class="financial-shape-fact"><dt>自己資本比率</dt><dd>${escapeHtml(formatMetricValue(latestValue(company, 'equityRatio'), 'equityRatio'))}</dd></div>`,
      '  </dl>',
      '</article>',
    ].join('\n');
  }
  ...
  function renderProfitShapeCard(company, maxRevenue) {
    ...
    return [
      ...
      revenue == null || revenue <= 0 ? '' : [
        '    <div class="financial-shape-total-badge">',
        '      <span>営業収益</span>',
        `      <strong>${escapeHtml(compactMetricValue(revenue, 'revenue'))}</strong>`,
        '    </div>',
      ].join('\n'),
      ...
      revenue == null || revenue <= 0 ? `      <span class="financial-shape-box__empty">${escapeHtml(revenue == null ? '売上未開示' : '営業収益がマイナス')}</span>` : [
        '      <span class="financial-shape-segment financial-shape-segment--cost" aria-hidden="true"></span>',
        operatingProfit != null && operatingProfit >= 0
          ? '      <span class="financial-shape-segment financial-shape-segment--profit" aria-hidden="true"></span>'
          : '      <span class="financial-shape-segment financial-shape-segment--loss" aria-hidden="true"></span>',
      ].join('\n'),
      '    </div>',
      '  </div>',
      '  <dl class="financial-shape-card__facts">',
      `    <div class="financial-shape-fact financial-shape-fact--liability"><dt>費用</dt><dd>${escapeHtml(operatingCost == null ? '-' : formatMetricValue(operatingCost, 'revenue'))}</dd></div>`,
      `    <div class="financial-shape-fact ${operatingProfit != null && operatingProfit < 0 ? 'financial-shape-fact--loss' : 'financial-shape-fact--equity'}"><dt>${escapeHtml(operatingProfit != null && operatingProfit < 0 ? '営業損失' : '営業利益')}</dt><dd>${escapeHtml(formatMetricValue(operatingProfit, 'operatingProfit'))}</dd></div>`,
      `    <div class="financial-shape-fact"><dt>営業利益率</dt><dd>${escapeHtml(formatMetricValue(latestValue(company, 'operatingMargin'), 'operatingMargin'))}</dd></div>`,
      '  </dl>',
      '</article>',
    ].join('\n');
  }
