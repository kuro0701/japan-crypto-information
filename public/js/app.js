document.addEventListener('DOMContentLoaded', () => {
  ...
  const skeletonCell = (widths, align = 'left') => `
    <span class="skeleton-cell${align === 'right' ? ' skeleton-cell--right' : ''}">
      ${widths.map((item) => {
        const config = typeof item === 'string' ? { width: item, className: '' } : item;
        return skeletonLine(config.width, config.className || '');
      }).join('')}
    </span>
  `;
  ...
  function renderDecisionSummaryCard({
    ...
    panel.className = [
      ...
      'mt-3',
      `beginner-conclusion-card--${status}`,
    ].join(' ');
    panel.innerHTML = `
      <div class="beginner-conclusion-card__header">
        <p class="beginner-conclusion-card__prefix">この条件なら</p>
        <span class="decision-summary-badge decision-summary-badge--${escapeHtml(badgeTone)}">${escapeHtml(badgeLabel)}</span>
      </div>
      <dl class="beginner-conclusion-list">
        <div>
          <dt>最安候補</dt>
          <dd>${escapeHtml(candidateText)}</dd>
        </div>
        <div>
          <dt>2位との差</dt>
  ...
  function resetDecisionSummary() {
    renderDecisionSummaryCard({
      ...
      candidate: '比較前',
      gap: '-',
      note: '買いたい銘柄、購入金額、注文方法を確認して「比較する」を押してください。',
    });
  }
  ...
  function setDecisionSummaryLoading() {
    ...
    renderDecisionSummaryCard({
      ...
      candidate: '計算中',
      gap: '計算中',
      note: '現在の板を使って、最安候補と2位との差を比較しています。',
      meta: `${market.label || market.instrumentId || '-'} / ${exchange.label || exchange.id}`,
    });
  }
  ...
  function setDecisionSummaryError(message) {
    renderDecisionSummaryCard({
      ...
      candidate: '要確認',
      gap: '-',
      note: message || '入力条件や接続状態を確認してください。',
    });
  }

  function positiveJpyLabel(value, suffix = '') {
  ...
  function venueRowsForConclusion(data = lastVenueComparisonData) {
    const rows = executableVenueComparisonRows(data);
    const sourceRows = rows.length > 0 ? rows : freshVenueComparisonRows(data);
    return sourceRows
      .filter(row => row && row.result && !row.result.error)
      .slice()
      .sort((a, b) => {
  ...
  function venueGapLabel(meta, bestRow, secondRow) {
    if (!meta || !bestRow || !secondRow || !bestRow.result || !secondRow.result) return '-';
    const best = bestRow.result;
    const second = secondRow.result;
    const isSell = meta.side === 'sell';
    const fixedQuote = meta.amountType === 'jpy';

    if (fixedQuote) {
      const bestBase = Number(best.totalBTCFilled);
      const secondBase = Number(second.totalBTCFilled);
      const price = Number(best.effectiveVWAP) || Number(second.effectiveVWAP);
      if (!Number.isFinite(bestBase) || !Number.isFinite(secondBase) || !Number.isFinite(price)) return '-';
      const baseAdvantage = isSell ? secondBase - bestBase : bestBase - secondBase;
      return positiveJpyLabel(baseAdvantage * price, '相当');
    }

    const bestQuote = Number(best.effectiveCostJPY);
    const secondQuote = Number(second.effectiveCostJPY);
    if (!Number.isFinite(bestQuote) || !Number.isFinite(secondQuote)) return '-';
    return positiveJpyLabel(isSell ? bestQuote - secondQuote : secondQuote - bestQuote);
  }

  function beginnerConclusionNote(meta, bestRow) {
    const result = bestRow && bestRow.result;
    const risk = result ? UI.impactRiskMeta(result.marketImpactPct) : null;
    const salesRows = Array.isArray(lastSalesReferenceData && lastSalesReferenceData.rows)
      ? lastSalesReferenceData.rows
      : [];
    const salesLooksWorse = salesRows.some((row) => {
      const disadvantage = Number(row && row.delta && row.delta.disadvantageJpy);
      return Number.isFinite(disadvantage) && disadvantage > 0.5;
    });
    ...
    if (!meta || meta.side !== 'sell') {
      if (salesLooksWorse || !lastSalesReferenceData) {
        return '販売所ではなく取引所形式の方が安い可能性があります';
      }
      return '販売所参考コストも確認済みです。最終注文前は公式画面の提示価格を確認してください。';
    }
  ...
  function updateDecisionSummary() {
    ...
    const conclusionRows = venueRowsForConclusion(lastVenueComparisonData);
    const executableRows = executableVenueComparisonRows(lastVenueComparisonData);

    if (!lastVenueComparisonData || conclusionRows.length === 0) {
      renderDecisionSummaryCard({
        status: 'loading',
        badgeLabel: '比較中',
        badgeTone: 'loading',
  ...
  function setChartLoading(isLoading) {
    if (!depthChartContainer) return;
    depthChartContainer.classList.toggle('is-loading', Boolean(isLoading));
    depthChartContainer.setAttribute('aria-busy', isLoading ? 'true' : 'false');
  }

  function setOverviewLoadingState() {
    const rows = [
      ['mid-price', [{ width: '72%', className: 'skeleton-line--hero' }]],
      ['best-bid', ['54%']],
      ['best-ask', ['58%']],
      ['spread-jpy', ['40%']],
      ['spread-pct', [{ width: '52%', className: 'skeleton-line--sm' }]],
      ['volume-24h-base', ['46%']],
      ['volume-24h-quote', ['62%']],
      ['ask-volume', ['44%']],
      ['bid-volume', ['44%']],
    ...
    rows.forEach(([id, widths]) => {
      setHtml(id, skeletonCell(widths, 'right'));
    });
  }

  function setThresholdLoading() {
    const tbody = document.getElementById('threshold-tbody');
    if (!tbody) return;
    tbody.innerHTML = Array.from({ length: 4 }, () => `
      <tr class="table-skeleton-row">
        <td headers="threshold-col-impact" class="is-num text-right" data-label="変動">${skeletonCell([{ width: '42%', className: 'skeleton-line--sm' }], 'right')}</td>
        <td headers="threshold-col-buy" class="is-num text-right" data-label="買い">${skeletonCell(['56%', '72%', '48%'], 'right')}</td>
        <td headers="threshold-col-sell" class="is-num text-right" data-label="売り">${skeletonCell(['56%', '72%', '48%'], 'right')}</td>
      </tr>
    `).join('');
  }

  function setSimulationLoading() {
    setBusy('simulation-export-panel', true);
    setHtml('simulation-results', `
      <div class="space-y-3">
        <div class="skeleton-block">
          ${skeletonLine('34%')}
          ${skeletonLine('82%', 'skeleton-line--sm')}
        </div>
        <div class="skeleton-block">
          <div class="skeleton-block__row">
            ${skeletonCell(['34%'])}
            ${skeletonCell(['58%'], 'right')}
          </div>
          <div class="skeleton-block__row">
            ${skeletonCell(['30%'])}
            ${skeletonCell(['64%'], 'right')}
          </div>
          <div class="skeleton-block__row">
            ${skeletonCell(['36%'])}
            ${skeletonCell(['52%'], 'right')}
          </div>
          <div class="skeleton-block__row">
            ${skeletonCell(['28%'])}
            ${skeletonCell(['60%'], 'right')}
          </div>
        </div>
      </div>
    `);
  ...
  function setFillLoading() {
    const tbody = document.getElementById('fill-tbody');
    if (!tbody) return;
    setBusy('fill-tbody', true);
    tbody.innerHTML = Array.from({ length: 4 }, () => `
      <tr class="table-skeleton-row">
        <td headers="fill-col-side" data-label="方向">${skeletonCell(['46%'])}</td>
        <td headers="fill-col-idx" class="is-num text-right" data-label="#">${skeletonCell(['26%'], 'right')}</td>
        <td headers="fill-col-price" class="is-num text-right" data-label="約定価格">${skeletonCell(['58%'], 'right')}</td>
        <td headers="fill-quantity-header" class="is-num text-right" data-label="数量">${skeletonCell(['48%'], 'right')}</td>
  ...
  function setVenueComparisonLoading() {
    const tbody = document.getElementById('venue-comparison-tbody');
    if (!tbody) return;
    setBusy('venue-comparison-panel', true);
    tbody.innerHTML = Array.from({ length: 4 }, () => `
      <tr class="table-skeleton-row">
  ...
  function setSalesReferenceLoading() {
    const tbody = document.getElementById('sales-reference-tbody');
    if (!tbody) return;
    setBusy('sales-reference-panel', true);
    tbody.innerHTML = Array.from({ length: 4 }, () => `
      <tr class="table-skeleton-row">
        <td headers="sales-ref-col-rank" class="is-num text-right" data-label="順位">${skeletonCell(['28%'], 'right')}</td>
        <td headers="sales-ref-col-exchange" data-label="販売所">${skeletonCell(['48%', '34%'])}</td>
        <td headers="sales-ref-col-price" class="is-num text-right" data-label="表示価格">${skeletonCell(['52%', '30%'], 'right')}</td>
        <td headers="sales-ref-col-result" class="is-num text-right" data-label="参考結果">${skeletonCell(['58%', '46%'], 'right')}</td>
        <td headers="sales-ref-col-delta" class="is-num text-right" data-label="最良取引所との差">${skeletonCell(['54%', '36%'], 'right')}</td>
        <td headers="sales-ref-col-risk" data-label="注意">${skeletonCell(['42%', '38%'])}</td>
        <td headers="sales-ref-col-updated" class="text-right" data-label="更新">${skeletonCell(['40%'], 'right')}</td>
      </tr>
    `).join('');
  }
  ...
  function openDetailDisclosure(options = {}) {
    ...
    if (scroll) {
      requestAnimationFrame(() => {
        detailDisclosure.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    }
  }
  ...
  function updateAmountUnit() {
    ...
    if (amountInputLabel) {
      if (isBaseAmount) {
        amountInputLabel.textContent = side === 'sell' ? '売却数量' : '購入数量';
      } else {
        amountInputLabel.textContent = side === 'sell' ? '売却金額' : '購入金額';
      }
    }
  ...
  function applyInitialUrlState() {
    ...
    const initialAmountType = initialUrlState.hasAmountType ? initialUrlState.amountType : 'jpy';
    ...
    if (!initialUrlState.hasAmount && initialAmountType === 'base' && amountInput && amountInput.value === amountInput.defaultValue) {
      amountInput.value = '';
    }
  ...
  document.addEventListener('click', (event) => {
    const button = closestFromEvent(event, '[data-open-detail]');
    if (!button) return;
    openDetailDisclosure();
  });
  ...
  document.addEventListener('click', (event) => {
    const button = closestFromEvent(event, '[data-export-kind][data-export-format]');
    if (!button) return;
